    ...
    maxRetrySequenceIndex(MaxNumberAccessCategories - 1)
{
    ...
    if (theParameterDatabaseReader.ParameterExists(
        ...
        const unsigned int maxNumMimoSpatialStreamsOfMac = 
            macDurationCalculationInterfacePtr->GetMaxNumMimoSpatialStreams();

        if (maxNumMimoSpatialStreams > maxNumMimoSpatialStreamsOfMac) {
            ...
            if (maxNumMimoSpatialStreams > 1) {
                cerr << "  Set \"dot11-allow-mimo-speed-transmissions-without-mimo-model = true\" or \"dot11-channel-model = mimo(or tgnmimo)\"" << endl;
            }//if//
