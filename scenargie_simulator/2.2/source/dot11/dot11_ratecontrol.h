    ...
    const InterfaceId& theInterfaceId)
{
    ...
    for(IterType iter = ackDatarateMap.begin(); (iter != ackDatarateMap.end()); ++iter) {
        ...
        GetModulationAndCodingSchemeFromName(iter->first, success, notUsed);
    ...
    while (mcs <= maxModulationAndCodingScheme) {
        ...
        GetModulationAndCodingSchemeFromName(mcsValueString, success, mcsValue);
    ...
    TransmissionParameters& txParameters) const
{
    ...
    // If dataframes will be wider than one channel, then replicate RTS/CTS on all channels.

    if (GetStationBandwidthNumChannels(macAddress) > 1) {
        txParameters.bandwidthNumberChannels =
            ((*this).GetMaxChannelBandwidthMhz() / (*this).GetBaseChannelBandwidthMhz());

        if (txParameters.bandwidthNumberChannels > 1) {
            txParameters.isAReplicatedSingleChannelSignal = true;
        }//if//
    }//if//
...
private:
    ...
    struct ModulationAndCodingSchemeEntry {
        ...
        //bool isShortGuardInterval; --> false: normal guard interval (in dot11_phy.h default)
    ...
    const unsigned int retrySequenceIndex)
{
    ...
    if (macAddress.IsABroadcastOrAMulticastAddress()) {
        ...
        // Broadcast frame MCS is static in the model.
        //   => No process for transmission rate calculation.
        
    }
    else {
       
        if ((*this).StationIsKnown(macAddress)) {
    ...
    const unsigned int retrySequenceIndex)
{
    ...
    const IterType iter = stationInfoForAddr.find(stationMacAddress);
    
    if (iter == stationInfoForAddr.end()) {
        cerr << "Minstrel Error: Station Information is not initialized" << endl;
        exit(1);
    }//if//
