...
#ifndef DOT11AD_MAC_H
#define DOT11AD_MAC_H
...
#include "dot11ad_txpowercontrol.h"
#include "dot11ad_mac_ap.h"
#include "dot11ad_mac_sta.h"
#include "dot11ad_incoming_buffer.h"
...
public:
    ...
    void SetCustomAdaptiveRateController(
        const shared_ptr<Dot11ad::AdaptiveRateController>& rateControllerPtr)
    {
        this->theAdaptiveRateControllerPtr = rateControllerPtr;
    }

    shared_ptr<Dot11ad::AdaptiveRateController> GetAdaptiveRateControllerPtr() const
    {
        return (theAdaptiveRateControllerPtr);
    }

    shared_ptr<Dot11ad::AdaptiveTxPowerController> GetAdaptiveTxPowerControllerPtr() const
    {
        return (theAdaptiveTxPowerControllerPtr);
    }
    ...
    MacAddress MakeMacAddressFromNodeId(const NodeId& aNodeId) const;
    ...
    unsigned int GetNumberAntennaSectors() const { return (physicalLayer.GetNumberAntennaSectors());}
    ...
    double GetSinrOfLastFrameDb() const { return (physicalLayer.GetSinrOfLastFrameDb()); }
    SimTime GetStartTimeOfLastFrame() const { return (physicalLayer.GetStartTimeOfLastFrame()); }
    SimTime GetDurationOfLastFrame() const { return (physicalLayer.GetDurationOfLastFrame()); }
    ...
    void LookupMacAddressForNeighbor(const NodeId theNodeId, bool& wasFound, MacAddress& macAddress);
    ...
    SimTime GetReceiveSweepPerSectorDuration() const { return (receiveSweepPerSectorDuration); }

    SimTime GetShortBeamformingInterframeSpaceDuration() const {
        return (physicalLayer.GetShortBeamformingInterframeSpaceDuration());
    }

    SimTime GetMediumBeamformingInterframeSpaceDuration() const {
        return (physicalLayer.GetMediumBeamformingInterframeSpaceDuration());
    }

    SimTime CalcAssociationBeamformingTrainingSlotTime(const unsigned int numberSweepFrames) const;
    SimTime CalculateSweepFrameDuration(const unsigned int frameSizeBytes) const;

    void StartContentionPeriod(const SimTime& anContentionPeriodEndTime);
    void StartNonContentionPeriod(const SimTime& anServicePeriodEndTime);

    void TransmitDirectionalSectorSweepFrame(
        unique_ptr<Packet>& framePtr,
    ...
    void SwitchToQuasiOmniAntennaMode() { physicalLayer.SwitchToQuasiOmniAntennaMode(true/*setRxPattern*/); }

    void SwitchToDirectionalAntennaMode(const unsigned int sectorId)
    {
        physicalLayer.SwitchToDirectionalAntennaMode(sectorId, true/*setRxPattern*/);
    }
...
private:
    ...
    SimTime accessPeriodEndTime;
    bool isInNonContentionAccessPeriod;

    SimTime receiveSweepPerSectorDuration;
    ...
    shared_ptr<CounterStatistic> sswFramesSentStatPtr;
    shared_ptr<CounterStatistic> sswFramesReceivedStatPtr;

    shared_ptr<CounterStatistic> sswFeedbackFramesSentStatPtr;
    shared_ptr<CounterStatistic> sswFeedbackFramesReceivedStatPtr;

    shared_ptr<CounterStatistic> sswAckFramesSentStatPtr;
    shared_ptr<CounterStatistic> sswAckFramesReceivedStatPtr;

    shared_ptr<CounterStatistic> dmgBeaconFramesSentStatPtr;
    shared_ptr<CounterStatistic> dmgBeaconFramesReceivedStatPtr;
    ...
    void TransmitFrameInNonContentionPeriod();
    ...
    void BuildAggregateFrameFromCurrentFrame(const SimTime& delayUntilTransmitting);
    ...
    void DoTransmissionTooLongForAccessPeriodAction(const unsigned int accessCategoryIndex);

    unsigned int GetTxAntennaSectorId(const MacAddress& destinationAddress) const;
    unsigned int GetRxAntennaSectorId(const MacAddress& destinationAddress) const;
    ...
    void OutputTraceAndStatsForDMGFrameTransmission(const Packet& aFrame) const;
...
Dot11Mac::Dot11Mac(
    ...
    accessPeriodEndTime(ZERO_TIME),
    isInNonContentionAccessPeriod(false),
    physicalLayer(
        theParameterDatabaseReader,
        theAntennaPatternDatabase,
        initNodeId,
    ...
    sswFramesSentStatPtr(
        simulationEngineInterfacePtr->CreateCounterStat(
            (modelName + '_' + theInterfaceId + "_SSW_FramesSent"))),
    sswFramesReceivedStatPtr(
        simulationEngineInterfacePtr->CreateCounterStat(
            (modelName + '_' + theInterfaceId + "_SSW_FramesReceived"))),
    sswFeedbackFramesSentStatPtr(
        simulationEngineInterfacePtr->CreateCounterStat(
            (modelName + '_' + theInterfaceId + "_SswFeedback_FramesSent"))),
    sswFeedbackFramesReceivedStatPtr(
        simulationEngineInterfacePtr->CreateCounterStat(
            (modelName + '_' + theInterfaceId + "_SswFeedback_FramesReceived"))),
    sswAckFramesSentStatPtr(
        simulationEngineInterfacePtr->CreateCounterStat(
            (modelName + '_' + theInterfaceId + "_SswAck_FramesSent"))),
    sswAckFramesReceivedStatPtr(
        simulationEngineInterfacePtr->CreateCounterStat(
            (modelName + '_' + theInterfaceId + "_SswAck_FramesReceived"))),
    dmgBeaconFramesSentStatPtr(
        simulationEngineInterfacePtr->CreateCounterStat(
            (modelName + '_' + theInterfaceId + "_DmgBeacon_FramesSent"))),
    dmgBeaconFramesReceivedStatPtr(
        simulationEngineInterfacePtr->CreateCounterStat(
            (modelName + '_' + theInterfaceId + "_DmgBeacon_FramesReceived"))),
            ...
            (modelName + '_' + theInterfaceId + "_Authentication_FramesReceived")))
{
    ...
    receiveSweepPerSectorDuration =
        (CalculateSweepFrameDuration(sizeof(SectorSweepFrameType)) +
         GetShortBeamformingInterframeSpaceDuration());

    // RTS/CTS is not supported (11ad not a broadcast/omnidirectional oriented technology)

    //11ad// if (theParameterDatabaseReader.ParameterExists(
    //11ad//         (parameterNamePrefix + "rts-threshold-size-bytes"), theNodeId, theInterfaceId)) {

    //11ad//     rtsThresholdSizeBytes =
    //11ad//         theParameterDatabaseReader.ReadNonNegativeInt(
    //11ad//             (parameterNamePrefix + "rts-threshold-size-bytes"), theNodeId, theInterfaceId);
    ...
    disallowAddingNewPacketsToRetries = true;
    ...
    myMacAddress = MakeMacAddressFromNodeId(primaryNodeId);
...
inline
MacAddress Dot11Mac::MakeMacAddressFromNodeId(const NodeId& aNodeId) const
{
    // Mac address related multiple interface restrictions:
    //   - Only one Adhoc interface on a single propagation environment.
    //   - Can't have two interfaces associate with same AP.
    ...
    aMacAddress.SetLowerBitsWithNodeId(aNodeId);

    if (operationMode == AdhocMode) {
        aMacAddress.SetInterfaceSelectorByte(AdhocModeAddressSelectorByte);
    }
    else {
        aMacAddress.SetInterfaceSelectorByte(static_cast<unsigned char>(interfaceIndex));
    }//if//

    return (aMacAddress);

}//MakeMacAddressFromNodeId//
    ...
    const RandomNumberGeneratorSeed& nodeSeed)
{
    ...
    case ApMode:
        apControllerPtr =
            Dot11ApManagementController::Create(
                shared_from_this(),
...
bool Dot11Mac::FrameIsForThisNode(const Packet& aFrame, const unsigned int headerOffset) const
{
    ...
    if (header.theFrameControlField.frameTypeAndSubtype == DMG_BEACON_FRAME_TYPE_CODE) {
        // DMG Beacons don't even have a "common header" (bssid (ap address) instead of receiverAddress).
        // Filter beacon frames from other bssid's later (treat as broadcast).
        return true;
    }//if//
    ...
    if ((header.receiverAddress == myMacAddress) ||
        ...
        case InfrastructureMode:
        {
            if (staControllerPtr->AllowDirectStaToStaReceive()) {
                isForMe = true;
            }
            else if (header.theFrameControlField.frameTypeAndSubtype == QOS_DATA_FRAME_TYPE_CODE) {
...
{
    assert(!isInNonContentionAccessPeriod);

    assert(macState != WaitingForIfsAndBackoffState);

    const SimTime currentTime = simEngineInterfacePtr->CurrentTime();

    if (currentTime >= accessPeriodEndTime) {
        // Not in a contention period, can't access medium.
        macState = IdleState;
        if (!wakeupTimerEventTicket.IsNull()) {
    ...
    const bool forceBackoff)
{
    ...
    case WaitingForIfsAndBackoffState:
    {
        ...
        if ((simEngineInterfacePtr->CurrentTime() < accessPeriodEndTime) &&
            (!isInNonContentionAccessPeriod)) {
...
void Dot11Mac::ClearChannelAtPhysicalLayerNotification()
{
    ...
    if (isInNonContentionAccessPeriod) {
        return;
    }//if//
    ...
    assert((macState == BusyMediumState) || (macState == TransientState) ||
           ((macState == IdleState) && (currentTime >= accessPeriodEndTime)));

    if (currentTime < mediumReservedUntilTimeAkaNAV) {
        (*this).GoIntoWaitForExpirationOfVirtualCarrierSenseAkaNavState();
    }
    else {
        mediumBecameIdleTime = currentTime;
        (*this).StartBackoffIfNecessary();
...
void Dot11Mac::DoSuccessfulTransmissionPostProcessing(const bool wasJustTransmitting)
{
    ...
    if (currentTime >= accessPeriodEndTime) {
        // This logic should happen only for sector sweep frames.
    ...
    if (isInNonContentionAccessPeriod) {
        ...
        (*this).TransmitFrameInNonContentionPeriod();
    }
    else {
        ...
        if ((currentTime + nextFrameSequenceDuration) > currentTransmitOpportunityAkaTxopEndTime) {
            ...
            if (!packetHasBeenSentToPhy) {

                // Either there were no frames to send or first frame sequence is too long for
                // the access period.
...
void Dot11Mac::TransmissionIsCompleteNotification()
{
    if (macState == WaitingForCtsState) {
    ...
    else if (macState == StartReceivingState) {
        ...
        // Do nothing.
        assert(simEngineInterfacePtr->CurrentTime() >= accessPeriodEndTime);
    }
    else {
        assert(macState == BusyMediumState);
...
inline
void Dot11Mac::BuildAggregateFrameFromCurrentFrame(const SimTime& delayUntilTransmitting)
{
    const SimTime currentTime = simEngineInterfacePtr->CurrentTime();
    ...
    endTime += max(
        ackTimeoutDuration,
        (CalculateNavDurationForAckedAggregateDataFrame(currentPacketsDestinationMacAddress, txParameters) +
         physicalLayer.GetAirPropagationTimeDuration()));
...
inline
SimTime Dot11Mac::CalculateSweepFrameDuration(const unsigned int frameSizeBytes) const
{
    TransmissionParameters txParameters;
    txParameters.bandwidthNumberChannels = 1;
    txParameters.firstChannelNumber = 0; // Doesn't matter for calculation.
    txParameters.isHighThroughputFrame = false;
    txParameters.modulationAndCodingScheme = AdMcs0;

    return (
        physicalLayer.CalculateFrameTransmitDuration(frameSizeBytes, txParameters));
}
...
void Dot11Mac::CalculateAndSetTransmitOpportunityDurationAkaTxop(const unsigned int accessCategoryIndex)
{
    ...
    if (macSchedulerPtr.get() != nullptr) {
        // 802.11ad Future possibility: Tell Scheduler about contention period end.
    ...
    if (accessCategoryInfo.transmitOpportunityDurationAkaTxop == ZERO_TIME) {
        ...
        if (currentTransmitOpportunityAkaTxopEndTime > accessPeriodEndTime) {
            (*this).currentTransmitOpportunityAkaTxopEndTime = accessPeriodEndTime;
        }//if//
    }//if//
    ...
    const SimTime& delayUntilTransmitting)
{
    ...
    physicalLayer.TransmitDirectionalAggregateFrame(
        currentAggregateFramePtr,
        txParameters,
        transmitPowerDbm,
        GetTxAntennaSectorId(currentPacketsDestinationMacAddress),
    ...
    bool& packetHasBeenSentToPhy)
{
    ...
    const double transmitPowerDbm =
        theAdaptiveTxPowerControllerPtr->GetCurrentTransmitPowerDbm(destinationAddress);
    ...
    if ((currentTransmitOpportunityAkaTxopEndTime != ZERO_TIME) ||
        (accessPeriodEndTime != INFINITE_TIME)) {
        ...
        const SimTime frameTransmissionSequenceEndTime =
            (currentTime + delayUntilTransmitting +
             CalculateFrameDuration(frameToSendPtr->LengthBytes(), txParameters) +
             physicalLayer.GetAirPropagationTimeDuration()) +
            max(
                ackTimeoutDuration,
        ...
        if (frameTransmissionSequenceEndTime >= accessPeriodEndTime) {
            // Enforce 802.11ad Contention/Access Period (don't transmit).
            (*this).DoTransmissionTooLongForAccessPeriodAction(currentPacketsAccessCategoryIndex);
    ...
    physicalLayer.TransmitDirectionalFrame(
        frameToSendPtr,
        txParameters,
        transmitPowerDbm,
        GetTxAntennaSectorId(destinationAddress),
        delayUntilTransmitting);
    ...
    bool& packetHasBeenSentToPhy)
{
    ...
    const double transmitPowerDbm =
        theAdaptiveTxPowerControllerPtr->GetCurrentTransmitPowerDbm(currentPacketsDestinationMacAddress);
    ...
    if ((!currentPacketIsAManagementFrame) &&
        ...
        if (!currentPacketsDestinationMacAddress.IsAMulticastAddress()) {
            ...
            if (linkInfo.blockAckRequestNeedsToBeSent) {
                (*this).currentPacketsAccessCategoryIndex = ConvertToPacketPriority(accessCategoryIndex);
                (*this).currentPacketsDestinationMacAddress = currentPacketsDestinationMacAddress;
        ...
        if (CanAggregateCurrentFrame()) {
            ...
            (*this).BuildAggregateFrameFromCurrentFrame(delayUntilTransmitting);
    ...
    if (currentPacketsDestinationMacAddress.IsABroadcastOrAMulticastAddress()) {
        ...
        if ((doNotRequestToSendAkaRts) ||
            ...
            if ((currentTransmitOpportunityAkaTxopEndTime != ZERO_TIME) ||
                ...
                const SimTime frameTransmissionSequenceEndTime =
                    (currentTime + delayUntilTransmitting +
                     CalculateFrameDuration(sizeof(RequestToSendFrame), txParameters) +
                     physicalLayer.GetAirPropagationTimeDuration()) +
                    max(
                        clearToSendTimeoutDuration,
                        CalculateNavDurationForRtsFrame(
                            currentPacketsDestinationMacAddress,
                ...
                if (frameTransmissionSequenceEndTime >= accessPeriodEndTime) {
                    // Enforce 802.11ad Contention/Access Period (don't transmit).
                    (*this).DoTransmissionTooLongForAccessPeriodAction(accessCategoryIndex);
                    packetHasBeenSentToPhy = false;
                    return;
                }//if//
...
inline
void Dot11Mac::TransmitFrameInNonContentionPeriod()
{
    assert(isInNonContentionAccessPeriod);
    ...
    for(int i = (numberAccessCategories - 1); (i >= 0); i--) {
        ...
        if (accessCategoryInfo.hasPacketToSend) {
           //11ad (*this).CalculateAndSetTransmitOpportunityDurationAkaTxop(accessCategoryIndex);
...
}//TransmitFrameInNonContentionPeriod//
...
void Dot11Mac::NeverReceivedClearToSendOrAcknowledgementAction()
{
    ...
    if ((!isInNonContentionAccessPeriod) && (lastSentFrameWasAn != SentFrameType::PowerSavePollResponse)) {
...
void Dot11Mac::ProcessClearToSendOrAcknowledgementTimeout()
{
    if (!physicalLayer.IsReceivingAFrame()) {
        ...
        assert(currentTime <= accessPeriodEndTime);
    ...
    const TransmissionParameters& receivedFrameTxParameters)
{
    ...
    const double transmitPowerDbm =
        theAdaptiveTxPowerControllerPtr->GetCurrentTransmitPowerDbm(ctsFrame.header.receiverAddress);
    ...
    physicalLayer.TransmitDirectionalFrame(
        ...
        transmitPowerDbm,
        GetTxAntennaSectorId(ctsFrame.header.receiverAddress),
        aShortInterframeSpaceTime);
    ...
    const TransmissionParameters& pollFrameTxParameters)
{
    ...
    const double transmitPowerDbm =
        theAdaptiveTxPowerControllerPtr->GetCurrentTransmitPowerDbm(psPollFrame.transmitterAddress);
...
void Dot11Mac::NotifyThatPhyReceivedCorruptedFrame()
{
    ...
    assert((macState == BusyMediumState) || (macState == IdleState));
}
...
void Dot11Mac::OutputTraceAndStatsForFrameReceive(const Packet& aFrame) const
{
    ...
    case DMG_BEACON_FRAME_TYPE_CODE: {
        dmgBeaconFramesReceivedStatPtr->IncrementCounter();
        break;
    }
    case SECTOR_SWEEP_FRAME_TYPE_CODE: {
        sswFramesReceivedStatPtr->IncrementCounter();
        break;
    }
    case SECTOR_SWEEP_FEEDBACK_FRAME_TYPE_CODE: {
        sswFeedbackFramesReceivedStatPtr->IncrementCounter();
        break;
    }
    case SECTOR_SWEEP_ACK_FRAME_TYPE_CODE: {
        sswAckFramesReceivedStatPtr->IncrementCounter();
        break;
    }
    default:
        assert(false); abort();
...
inline
void Dot11Mac::OutputTraceAndStatsForDMGFrameTransmission(const Packet& aFrame) const
{
    const CommonFrameHeader& header =
    ...
    if (simEngineInterfacePtr->TraceIsOn(TraceMac)) {
        ...
        NodeId destNodeId = 0;
        if ((frameType != DMG_BEACON_FRAME_TYPE_CODE) && (!header.receiverAddress.IsABroadcastOrAMulticastAddress())) {
            destNodeId = header.receiverAddress.ExtractNodeId();
        }//if//

        if (simEngineInterfacePtr->BinaryOutputIsOn()) {


            Dot11MacTxDMGTraceRecord traceData;
            ...
            assert(sizeof(traceData) == DOT11_MAC_TX_DMG_TRACE_RECORD_BYTES);

            simEngineInterfacePtr->OutputTraceInBinary(modelName, theInterfaceId, "Tx-DMG", traceData);
        }
        else {
            ...
            simEngineInterfacePtr->OutputTrace(
                modelName, theInterfaceId, "Tx-DMG", msgStream.str());
    ...
    switch (frameType) {
    case DMG_BEACON_FRAME_TYPE_CODE:
        dmgBeaconFramesSentStatPtr->IncrementCounter();
        break;
    case SECTOR_SWEEP_FRAME_TYPE_CODE:
        sswFramesSentStatPtr->IncrementCounter();
        break;
    case SECTOR_SWEEP_FEEDBACK_FRAME_TYPE_CODE:
        sswFeedbackFramesSentStatPtr->IncrementCounter();
        break;
    case SECTOR_SWEEP_ACK_FRAME_TYPE_CODE:
        sswAckFramesSentStatPtr->IncrementCounter();
        break;
    default:
        assert(false); abort();
        break;
...
}//OutputTraceAndStatsForDMGFrameTransmission//
    ...
    const TransmissionParameters& receivedFrameTxParameters)
{
    ...
    const CommonFrameHeader& theCommonFrameHeader =
        managementFrame.GetAndReinterpretPayloadData<CommonFrameHeader>();

    if (!IsADirectionalMgFrameTypeCode(theCommonFrameHeader.theFrameControlField.frameTypeAndSubtype)) {
        if (!theCommonFrameHeader.receiverAddress.IsABroadcastOrAMulticastAddress()) {
    ...
    const TransmissionParameters& receivedFrameTxParameters)
{
    ...
    if (FrameIsForThisNode(aFrame)) {
        ...
        if (currentTime >= accessPeriodEndTime) {
            ...
            // 802.11ad
            // EDCAF operates only during CBAPs.
            // NAV timer is unchanged until the start of next CBAP.
        ...
        if (header.theFrameControlField.frameTypeAndSubtype != POWER_SAVE_POLL_FRAME_TYPE_CODE) {
            endOfDurationTime =
                currentTime + (header.duration * MICRO_SECOND);
        }
        else {
...
void Dot11Mac::LookupMacAddressForNeighbor(
    const NodeId neighborNodeId, bool& wasFound, MacAddress& macAddress)
{
    switch (GetOperationMode()) {
    case InfrastructureMode: {
        // Allows for 11ad AP bypass:

        const MacAddress destinationMacAddress = MakeMacAddressFromNodeId(neighborNodeId);

        staControllerPtr->GetNextHopMacAddress(
            destinationMacAddress,
            wasFound,
            macAddress);
        break;
    ...
    case ApMode:
        apControllerPtr->LookupAssociatedNodeMacAddress(neighborNodeId, wasFound, macAddress);
        break;
    case AdhocMode:
        macAddress = MakeMacAddressFromNodeId(neighborNodeId);
        wasFound = true;
        break;
    default:
        assert(false); abort(); break;
...
inline
void Dot11Mac::StartContentionPeriod(const SimTime& anContentionPeriodEndTime)
{
    // Shorten the contention period by the 802.11 air propagation constant to make sure
    // the frames have not only stopped transmitting, but have been fully propagated.
    // Further shortened to make sure ack Timeouts are fielded.

    (*this).isInNonContentionAccessPeriod = false;

    (*this).accessPeriodEndTime = anContentionPeriodEndTime;

    //  physicalLayer.GetAnAirPropagationTime());
...
}//StartContentionPeriod()//



inline
void Dot11Mac::StartNonContentionPeriod(const SimTime& anServicePeriodEndTime)
{
    // Have exclusive access to transmit.

    // Shorten the access period by the 802.11 air propagation constant to make sure
    // the frames have not only stopped transmitting, but have been fully propagated.
    // Further shortened to make sure ack Timeouts are fielded.

    (*this).isInNonContentionAccessPeriod = true;
    (*this).accessPeriodEndTime = anServicePeriodEndTime;

    if (physicalLayer.IsNotReceivingFrames()) {
        physicalLayer.StartReceivingFrames();
    }//if//

    if (macState != BusyMediumState) {
...
}//StartNonContentionPeriod//



inline
void Dot11Mac::DoTransmissionTooLongForAccessPeriodAction(const unsigned int accessCategoryIndex)
{
    // End the current access period.
    (*this).accessPeriodEndTime = simEngineInterfacePtr->CurrentTime();
    (*this).macState = IdleState;
    if (!wakeupTimerEventTicket.IsNull()) {
        (*this).CancelWakeupTimer();
    }//if//

    // Regenerate new random backoff using current contention window for next contention period.
...
}//DoTransmissionTooLongForAccessPeriodAction//



inline
void Dot11Mac::TransmitDirectionalSectorSweepFrame(
    unique_ptr<Packet>& framePtr,
    const unsigned int sectorId,
    const double& transmitPowerDbm,
    const SimTime& delayUntilAirborne,
    SimTime& transmissionEndTime)
{
    if (macState == BusyMediumState) {
        ...
        // Maybe receiving a Sector Sweeep frame from other BSS on the same channel.
        // => Ignore receiving signal.
    }
    else {

        assert(macState == IdleState);

     }//if//

    TransmissionParameters txParameters;

    // Set and expand for 802.11ad.
    txParameters.bandwidthNumberChannels = 1;
    txParameters.isHighThroughputFrame = false;
    txParameters.modulationAndCodingScheme = AdMcs0;

    OutputTraceAndStatsForDMGFrameTransmission(*framePtr);

    physicalLayer.TransmitDirectionalFrame(
        framePtr, txParameters, transmitPowerDbm, sectorId, delayUntilAirborne, transmissionEndTime);

}//TransmitDirectionalSectorSweepFrame//



inline
unsigned int Dot11Mac::GetTxAntennaSectorId(const MacAddress& destinationAddress) const
{
    if (apControllerPtr != nullptr) {
        return (apControllerPtr->GetTxAntennaSectorId(destinationAddress));
    }
    else {
        assert(staControllerPtr != nullptr);
        return (staControllerPtr->GetTxAntennaSectorId(destinationAddress));
    }//if//
...
inline
void Dot11Mac::StopReceivingFrames()
{
    if (!IsNotReceivingFrames()) {

        if ((macState == WaitingForCtsState) || (macState == WaitingForAckState)) {
            // Should only happen if interference level kept the medium busy
            // until the end of the access interval.
...
inline
void Dot11Mac::StartReceivingFrames()
{
    if (IsNotReceivingFrames()) {

        if (physicalLayer.IsTransmittingAFrame()) {

            (*this).macState = StartReceivingState;
        }
        else {
            physicalLayer.StartReceivingFrames();
        }//if//
...
inline
SimTime Dot11Mac::CalcAssociationBeamformingTrainingSlotTime(
    const unsigned int numberSweepFrames) const
{
    const SimTime sweepFrameDuration = CalculateSweepFrameDuration(sizeof(SectorSweepFrameType));

    assert(numberSweepFrames > 0);

    const SimTime sectorSweepDuration =
        ((numberSweepFrames * sweepFrameDuration) +
         ((numberSweepFrames - 1) * GetShortBeamformingInterframeSpaceDuration()));

    const SimTime sweepFeedbackFrameDuration =
        CalculateSweepFrameDuration(sizeof(SectorSweepFeedbackFieldType));

    return (
        aShortInterframeSpaceTime +
        sectorSweepDuration +
        GetMediumBeamformingInterframeSpaceDuration() +
        sweepFeedbackFrameDuration +
        GetMediumBeamformingInterframeSpaceDuration());

}//CalcAssociationBeamformingTrainingSlotTime//


//Future inline
//Future unsigned int Dot11Mac::CalcMaxNumBeamformingSectorSweepFramesGivenSlotTime(
//Future     const SimTime& slotDuration) const
//Future {
//Future     const SimTime sweepFrameDuration = CalculateSweepFrameDuration(sizeof(SectorSweepFrameType));
//Future
//Future     const SimTime sweepFeedbackFrameDuration =
//Future         CalculateSweepFrameDuration(sizeof(SectorSweepFeedbackFieldType));
//Future
//Future     const SimTime feedbackAndOverheadDuration =
//Future         aShortInterframeSpaceTime +
//Future         GetMediumBeamformingInterframeSpaceDuration() +
//Future         sweepFeedbackFrameDuration +
//Future         GetMediumBeamformingInterframeSpaceDuration();
//Future
//Future     assert(feedbackAndOverheadDuration < slotDuration);
//Future
//Future     const SimTime timeForForSectorSweeps = slotDuration - feedbackAndOverheadDuration;
//Future
//Future     // Returns maximum number of sweep frames that will fit (rounded down).
//Future
//Future     const unsigned int maxNumSweepFrames = static_cast<unsigned int>(
//Future         (timeForForSectorSweeps + GetShortBeamformingInterframeSpaceDuration()) /
//Future         (sweepFrameDuration + GetShortBeamformingInterframeSpaceDuration()));
//Future
//Future     assert(CalcMaxNumBeamformingSectorSweepFramesGivenSlotTime(maxNumSweepFrames) <= slotDuration);
//Future
//Future     return (maxNumSweepFrames);
//Future
//Future }//CalcMaxNumBeamformingSectorSweepFramesGivenSlotTime//
...
inline
void CheckSectorIdMap(
    const ParameterDatabaseReader& theParameterDatabaseReader,
    const unsigned int numberAntennaSectors,
    const map<NodeId, unsigned int>& theMap,
    const string& parameterNameString)
{
    typedef map<NodeId, unsigned int>::const_iterator IterType;

    for(IterType iter = theMap.begin(); (iter != theMap.end()); ++iter) {
        const NodeId theNodeId = iter->first;
        const unsigned int sectorId = iter->second;

        if (sectorId >= numberAntennaSectors) {
            cerr << "Error in: \"" << parameterNameString << "\" parameter: Bad sector ID." << endl;
            exit(1);
        }//if//

        if (!theParameterDatabaseReader.CommNodeIdExists(theNodeId)) {
            cerr << "Warning in: \"" << parameterNameString << "\" parameter: Node ID" << theNodeId
                 << "  does not exist." << endl;
        }//if//
    }//for//
