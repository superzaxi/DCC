...
//ScenSim-Port//omapi_object_type_t *dhcp_type_interface;
//ScenSim-Port//omapi_object_type_t *dhcp_type_group;
//ScenSim-Port//omapi_object_type_t *dhcp_type_shared_network;
//ScenSim-Port//omapi_object_type_t *dhcp_type_subnet;
//ScenSim-Port//omapi_object_type_t *dhcp_type_control;
...
//ScenSim-Port//		log_fatal ("Can't register control object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register control object type: %i", status);//ScenSim-Port//
	status = dhcp_control_allocate (&dhcp_control_object, MDL);
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't make initial control object: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't make initial control object: %i", status);//ScenSim-Port//
	...
	status = omapi_object_type_register (&dhcp_type_group,
					     ...
					     RC_MISC);
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register group object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register group object type: %i", status);//ScenSim-Port//
	...
	status = omapi_object_type_register (&dhcp_type_subnet,
					     ...
					     RC_MISC);
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register subnet object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register subnet object type: %i", status);//ScenSim-Port//
	...
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register shared network object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register shared network object type: %i", status);//ScenSim-Port//
...
//ScenSim-Port//			group -> name = dmalloc (value -> u.buffer.len + 1,
			group -> name = (char*)dmalloc (value -> u.buffer.len + 1,//ScenSim-Port//
						 MDL);
			if (!group -> name)
	...
	if (!strcmp (name, "updated")) {
		...
		if (!group -> name) {
			...
			sprintf (hnbuf, "ng%08lx%08lx",
				 ...
				 (unsigned long)group);
//ScenSim-Port//			group -> name = dmalloc (strlen (hnbuf) + 1, MDL);
			group -> name = (char*)dmalloc (strlen (hnbuf) + 1, MDL);//ScenSim-Port//
			if (!group -> name)
				return ISC_R_NOMEMORY;
				      ...
				      omapi_typed_data_t *value)
{
	...
	if (!omapi_ds_strcmp (name, "state")) {
		...
		if (status != ISC_R_SUCCESS)
			return status;
//ScenSim-Port//		status = dhcp_set_control_state (control -> state, newstate);
		status = dhcp_set_control_state (control -> state, (control_object_state_t)newstate);//ScenSim-Port//
		if (status == ISC_R_SUCCESS)
//ScenSim-Port//			control -> state = value -> u.integer;
			control -> state = (control_object_state_t)value -> u.integer;//ScenSim-Port//
		return status;
	}
...
//ScenSim-Port//#if defined (DEBUG_MEMORY_LEAKAGE) || \
//ScenSim-Port//		defined (DEBUG_MEMORY_LEAKAGE_ON_EXIT)
//ScenSim-Port//	subnet = (struct subnet *)h;
//ScenSim-Port//	if (subnet -> next_subnet)
//ScenSim-Port//		subnet_dereference (&subnet -> next_subnet, file, line);
//ScenSim-Port//	if (subnet -> next_sibling)
//ScenSim-Port//		subnet_dereference (&subnet -> next_sibling, file, line);
//ScenSim-Port//	if (subnet -> shared_network)
//ScenSim-Port//		shared_network_dereference (&subnet -> shared_network,
//ScenSim-Port//					    file, line);
//ScenSim-Port//	if (subnet -> interface)
//ScenSim-Port//		interface_dereference (&subnet -> interface, file, line);
//ScenSim-Port//	if (subnet -> group)
//ScenSim-Port//		group_dereference (&subnet -> group, file, line);
//ScenSim-Port//#endif
...
//ScenSim-Port//#if defined (DEBUG_MEMORY_LEAKAGE) || \
//ScenSim-Port//    defined (DEBUG_MEMORY_LEAKAGE_ON_EXIT)
//ScenSim-Port//	struct shared_network *shared_network;
//ScenSim-Port//#endif
...
//ScenSim-Port//#if defined (DEBUG_MEMORY_LEAKAGE) || \
//ScenSim-Port//		defined (DEBUG_MEMORY_LEAKAGE_ON_EXIT)
//ScenSim-Port//	shared_network = (struct shared_network *)h;
//ScenSim-Port//	if (shared_network -> next)
//ScenSim-Port//		shared_network_dereference (&shared_network -> next,
//ScenSim-Port//					    file, line);
//ScenSim-Port//	if (shared_network -> name) {
//ScenSim-Port//		dfree (shared_network -> name, file, line);
//ScenSim-Port//		shared_network -> name = 0;
//ScenSim-Port//	}
//ScenSim-Port//	if (shared_network -> subnets)
//ScenSim-Port//		subnet_dereference (&shared_network -> subnets, file, line);
//ScenSim-Port//	if (shared_network -> interface)
//ScenSim-Port//		interface_dereference (&shared_network -> interface,
//ScenSim-Port//				       file, line);
//ScenSim-Port//	if (shared_network -> pools)
//ScenSim-Port//	    omapi_object_dereference ((omapi_object_t **)
//ScenSim-Port//				      &shared_network -> pools, file, line);
//ScenSim-Port//	if (shared_network -> group)
//ScenSim-Port//		group_dereference (&shared_network -> group, file, line);
//ScenSim-Port//#if defined (FAILOVER_PROTOCOL)
//ScenSim-Port//	if (shared_network -> failover_peer)
//ScenSim-Port//	    omapi_object_dereference ((omapi_object_t **)
//ScenSim-Port//				      &shared_network -> failover_peer,
//ScenSim-Port//				      file, line);
//ScenSim-Port//#endif
//ScenSim-Port//#endif /* DEBUG_MEMORY_LEAKAGE */
