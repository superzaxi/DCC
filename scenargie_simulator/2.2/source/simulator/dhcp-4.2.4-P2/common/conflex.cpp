...
//ScenSim-Port//isc_result_t new_parse (cfile, file, inbuf, buflen, name, eolp)
//ScenSim-Port//	struct parse **cfile;
//ScenSim-Port//	int file;
//ScenSim-Port//	char *inbuf;
//ScenSim-Port//	unsigned buflen;
//ScenSim-Port//	const char *name;
//ScenSim-Port//	int eolp;
isc_result_t new_parse (struct parse **cfile, int file, char *inbuf, unsigned buflen, const char *name, int eolp)//ScenSim-Port//
{
	isc_result_t status = ISC_R_SUCCESS;
	struct parse *tmp;

//ScenSim-Port//	tmp = dmalloc(sizeof(struct parse), MDL);
	tmp = (parse*)dmalloc(sizeof(struct parse), MDL);//ScenSim-Port//
	if (tmp == NULL) {
...
//ScenSim-Port//		tmp->inbuf = mmap(NULL, tmp->bufsiz, PROT_READ, MAP_SHARED,
		tmp->inbuf = (char*)mmap(NULL, tmp->bufsiz, PROT_READ, MAP_SHARED,//ScenSim-Port//
				  file, 0);
...
//ScenSim-Port//isc_result_t end_parse (cfile)
//ScenSim-Port//	struct parse **cfile;
isc_result_t end_parse (struct parse **cfile)//ScenSim-Port//
{
...
save_parse_state(struct parse *cfile) {
	 ...
	 */
//ScenSim-Port//	cfile->saved_state = dmalloc(sizeof(struct parse), MDL);
	cfile->saved_state = (parse*)dmalloc(sizeof(struct parse), MDL);//ScenSim-Port//
	if (cfile->saved_state == NULL) {
		return ISC_R_NOMEMORY;
	}
...
//ScenSim-Port//static int get_char (cfile)
//ScenSim-Port//	struct parse *cfile;
static int get_char (struct parse *cfile)//ScenSim-Port//
{
	/* My kingdom for WITH... */
	int c;
...
//ScenSim-Port//#if !defined(LDAP_CONFIGURATION)
		c = EOF;
//ScenSim-Port//#else /* defined(LDAP_CONFIGURATION) */
//ScenSim-Port//		if (cfile->read_function != NULL)
//ScenSim-Port//			c = cfile->read_function(cfile);
//ScenSim-Port//		else
//ScenSim-Port//			c = EOF;
//ScenSim-Port//#endif
	} else {
...
get_raw_token(struct parse *cfile) {
	...
	do {
		...
		if ((isascii (c) && isdigit (c)) || c == '-') {
			...
			cfile -> tlen = 1;
//ScenSim-Port//			ttok = c;
			ttok = (dhcp_token)c;//ScenSim-Port//
			break;
		}
	} while (1);
	return ttok;
...
get_next_token(const char **rval, unsigned *rlen, 
	...
	if (cfile -> token) {
		...
		rv = cfile -> token;
//ScenSim-Port//		cfile -> token = 0;
		cfile -> token = (dhcp_token)0;//ScenSim-Port//
	} else {
	...
	if (rlen)
		*rlen = cfile -> tlen;
//ScenSim-Port//#ifdef DEBUG_TOKENS
//ScenSim-Port//	fprintf (stderr, "%s:%d ", cfile -> tval, rv);
//ScenSim-Port//#endif
//ScenSim-Port//	return rv;
	return (dhcp_token)rv;//ScenSim-Port//
}
...
do_peek_token(const char **rval, unsigned int *rlen,
	...
	if (rlen)
		*rlen = cfile -> tlen;
//ScenSim-Port//#ifdef DEBUG_TOKENS
//ScenSim-Port//	fprintf (stderr, "(%s:%d) ", cfile -> tval, cfile -> token);
//ScenSim-Port//#endif
	return cfile -> token;
}
...
//ScenSim-Port//static void skip_to_eol (cfile)
//ScenSim-Port//	struct parse *cfile;
static void skip_to_eol (struct parse *cfile)//ScenSim-Port//
{
	int c;
	do {
		c = get_char (cfile);
...
//ScenSim-Port//static enum dhcp_token read_string (cfile)
//ScenSim-Port//	struct parse *cfile;
static enum dhcp_token read_string (struct parse *cfile)//ScenSim-Port//
{
	int i;
	int bs = 0;
	int c;
...
//ScenSim-Port//static enum dhcp_token read_number (c, cfile)
//ScenSim-Port//	int c;
//ScenSim-Port//	struct parse *cfile;
static enum dhcp_token read_number (int c, struct parse *cfile)//ScenSim-Port//
{
	int i = 0;
	int token = NUMBER;
...
//ScenSim-Port//		return token;
		return (dhcp_token)token;//ScenSim-Port//
}

//ScenSim-Port//static enum dhcp_token read_num_or_name (c, cfile)
//ScenSim-Port//	int c;
//ScenSim-Port//	struct parse *cfile;
static enum dhcp_token read_num_or_name (int c, struct parse *cfile)//ScenSim-Port//
{
	int i = 0;
