...
//ScenSim-Port//#ifdef HAVE_REGEX_H
//ScenSim-Port//# include <regex.h>
//ScenSim-Port//#endif
#define new new_ptr//ScenSim-Port//

//ScenSim-Port//struct binding_scope *global_scope;
...
//ScenSim-Port//pair cons (car, cdr)
//ScenSim-Port//	caddr_t car;
//ScenSim-Port//	pair cdr;
pair cons (caddr_t car, pair cdr)//ScenSim-Port//
{
...
//ScenSim-Port//int make_const_option_cache (oc, buffer, data, len, option, file, line)
//ScenSim-Port//	struct option_cache **oc;
//ScenSim-Port//	struct buffer **buffer;
...
int make_const_option_cache (struct option_cache **oc, struct buffer **buffer, u_int8_t *data, unsigned len, struct option *option, const char *file, int line)//ScenSim-Port//
{
	struct buffer *bp;
...
//ScenSim-Port//int make_host_lookup (expr, name)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	const char *name;
int make_host_lookup (struct expression **expr, const char *name)//ScenSim-Port//
{
...
//ScenSim-Port//int enter_dns_host (dh, name)
//ScenSim-Port//	struct dns_host_entry **dh;
//ScenSim-Port//	const char *name;
int enter_dns_host (struct dns_host_entry **dh, const char *name)//ScenSim-Port//
{
...
//ScenSim-Port//int make_const_int (expr, val)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	unsigned long val;
int make_const_int (struct expression **expr, unsigned long val)//ScenSim-Port//
{
	if (!expression_allocate (expr, MDL)) {
...
//ScenSim-Port//int make_concat (expr, left, right)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	struct expression *left, *right;
int make_concat (struct expression **expr, struct expression *left, struct expression *right)//ScenSim-Port//
{
...
//ScenSim-Port//int make_encapsulation (expr, name)
//ScenSim-Port//	struct expression **expr;
//ScenSim-Port//	struct data_string *name;
int make_encapsulation (struct expression **expr, struct data_string *name)//ScenSim-Port//
{
...
//ScenSim-Port//int make_substring (new, expr, offset, length)
//ScenSim-Port//	struct expression **new;
//ScenSim-Port//	struct expression *expr;
//ScenSim-Port//	struct expression *offset;
//ScenSim-Port//	struct expression *length;
int make_substring (struct expression **new, struct expression *expr, struct expression *offset, struct expression *length)//ScenSim-Port//
{
...
//ScenSim-Port//int make_limit (new, expr, limit)
//ScenSim-Port//	struct expression **new;
//ScenSim-Port//	struct expression *expr;
//ScenSim-Port//	int limit;
int make_limit (struct expression **new, struct expression *expr, int limit)//ScenSim-Port//
{
...
//ScenSim-Port//int make_let (result, name)
//ScenSim-Port//	struct executable_statement **result;
//ScenSim-Port//	const char *name;
int make_let (struct executable_statement **result, const char *name)//ScenSim-Port//
{
	if (!(executable_statement_allocate (result, MDL)))
		return 0;
	
	(*result) -> op = let_statement;
//ScenSim-Port//	(*result) -> data.let.name = dmalloc (strlen (name) + 1, MDL);
	(*result) -> data.let.name = (char*)dmalloc (strlen (name) + 1, MDL);//ScenSim-Port//
...
//ScenSim-Port//static int do_host_lookup (result, dns)
//ScenSim-Port//	struct data_string *result;
//ScenSim-Port//	struct dns_host_entry *dns;
static int do_host_lookup (struct data_string *result, struct dns_host_entry *dns)//ScenSim-Port//
{
	struct hostent *h;
	unsigned i, count;
	unsigned new_len;
...
//ScenSim-Port//int evaluate_expression (result, packet, lease, client_state,
//ScenSim-Port//			 in_options, cfg_options, scope, expr, file, line)
//ScenSim-Port//	struct binding_value **result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct client_state *client_state;
//ScenSim-Port//	struct option_state *in_options;
//ScenSim-Port//	struct option_state *cfg_options;
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct expression *expr;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int evaluate_expression (struct binding_value **result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct expression *expr, const char *file, int line)//ScenSim-Port//
{
	...
	if (expr -> op == expr_variable_reference) {
		...
		while (arg && s) {
//ScenSim-Port//			nb = dmalloc (sizeof *nb, MDL);
			nb = (struct binding*)dmalloc (sizeof *nb, MDL);//ScenSim-Port//
			if (!nb) {
			      blb:
...
//ScenSim-Port//				nb -> name = dmalloc (strlen (s -> string) + 1,
				nb -> name = (char*)dmalloc (strlen (s -> string) + 1,//ScenSim-Port//
						      MDL);
				if (nb -> name)
...
#if defined (NSUPDATE_OLD)
//ScenSim-Port//int evaluate_dns_expression (result, packet, lease, client_state, in_options,
//ScenSim-Port//			     cfg_options, scope, expr)
//ScenSim-Port//	ns_updrec **result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct client_state *client_state;
//ScenSim-Port//	struct option_state *in_options;
...
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct expression *expr;
int evaluate_dns_expression (ns_updrec **result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct expression *expr)//ScenSim-Port//
{
	unsigned long ttl = 0;
	char *tname;
...
//ScenSim-Port//			tname = dmalloc (name.len + 1, MDL);
			tname = dmalloc (name.len + 1, MDL);//ScenSim-Port//
			if (!tname) {
				r2 = 0;
				r1 = 0;
		...
		if (r0 && r1 && (r2 || expr -> op != expr_ns_add)) {
		    *result = minires_mkupdrec (((expr -> op == expr_ns_add ||
			...
			if (data.len) {
				...
				if (data.len == 4) {
				    (*result) -> r_data_ephem =
//ScenSim-Port//					    dmalloc (16, MDL);
					    dmalloc (16, MDL);//ScenSim-Port//
				    ...
				    (*result) -> r_data_ephem =
//ScenSim-Port//					    dmalloc (data.len, MDL);
					    dmalloc (data.len, MDL);//ScenSim-Port//
				    if (!(*result) -> r_data_ephem) {
...
//ScenSim-Port//int evaluate_boolean_expression (result, packet, lease, client_state,
//ScenSim-Port//				 in_options, cfg_options, scope, expr)
...
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct expression *expr;
int evaluate_boolean_expression (int *result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct expression *expr)//ScenSim-Port//
{
	...
	switch (expr -> op) {
	      case expr_check:
//ScenSim-Port//		*result = check_collection (packet, lease,
//ScenSim-Port//					    expr -> data.check);
		*result = curctx->check_collection (packet, lease, expr -> data.check);//ScenSim-Port//
		...
		*result = 0;
		...
		sleft = evaluate_boolean_expression (&bleft, packet, lease,
						     ...
						     in_options, cfg_options,
						     scope,
//ScenSim-Port//						     expr -> data.and [0]);
						     expr -> data.and_ [0]);//ScenSim-Port//
		if (sleft && bleft)
			sright = evaluate_boolean_expression
				(&bright, packet, lease, client_state,
				 in_options, cfg_options,
//ScenSim-Port//				 scope, expr -> data.and [1]);
				 scope, expr -> data.and_ [1]);//ScenSim-Port//
		else
			sright = bright = 0;
		...
		sleft = evaluate_boolean_expression (&bleft, packet, lease,
						     ...
						     in_options, cfg_options,
						     scope,
//ScenSim-Port//						     expr -> data.or [0]);
						     expr -> data.or_ [0]);//ScenSim-Port//
		if (!sleft || !bleft)
			sright = evaluate_boolean_expression
				(&bright, packet, lease, client_state,
				 in_options, cfg_options,
//ScenSim-Port//				 scope, expr -> data.or [1]);
				 scope, expr -> data.or_ [1]);//ScenSim-Port//
		else
			sright = 0;
#if defined (DEBUG_EXPRESSIONS)
		...
		sleft = evaluate_boolean_expression (&bleft, packet, lease,
						     ...
						     in_options, cfg_options,
						     scope,
//ScenSim-Port//						     expr -> data.not);
						     expr -> data.not_);//ScenSim-Port//
#if defined (DEBUG_EXPRESSIONS)
...
//ScenSim-Port//int evaluate_data_expression (result, packet, lease, client_state,
//ScenSim-Port//			      in_options, cfg_options, scope, expr, file, line)
//ScenSim-Port//	struct data_string *result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
...
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int evaluate_data_expression (struct data_string *result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct expression *expr, const char *file, int line)//ScenSim-Port//
{
	...
	switch (expr -> op) {
		...
		if (packet && packet -> raw -> file [0]) {
			char *fn =
//ScenSim-Port//				memchr (packet -> raw -> file, 0,
				(char*)memchr (packet -> raw -> file, 0,//ScenSim-Port//
		...
		if (packet && packet -> raw -> sname [0]) {
			char *fn =
//ScenSim-Port//				memchr (packet -> raw -> sname, 0,
				(char*)memchr (packet -> raw -> sname, 0,//ScenSim-Port//
...
//ScenSim-Port//int evaluate_numeric_expression (result, packet, lease, client_state,
//ScenSim-Port//				 in_options, cfg_options, scope, expr)
//ScenSim-Port//	unsigned long *result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct client_state *client_state;
//ScenSim-Port//	struct option_state *in_options;
...
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct expression *expr;
int evaluate_numeric_expression (unsigned long *result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct expression *expr)//ScenSim-Port//
{
...
//ScenSim-Port//						     expr -> data.and [0]);
		...
		sright = evaluate_numeric_expression (&iright, packet, lease,
						      ...
						      in_options, cfg_options,
						      scope,
//ScenSim-Port//						      expr -> data.and [1]);
						      expr -> data.and_ [1]);//ScenSim-Port//
...
//ScenSim-Port//int evaluate_option_cache (result, packet, lease, client_state,
//ScenSim-Port//			   in_options, cfg_options, scope, oc, file, line)
//ScenSim-Port//	struct data_string *result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct client_state *client_state;
...
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int evaluate_option_cache (struct data_string *result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct option_cache *oc, const char *file, int line)//ScenSim-Port//
{
...
//ScenSim-Port//int evaluate_boolean_option_cache (ignorep, packet,
//ScenSim-Port//				   lease, client_state, in_options,
//ScenSim-Port//				   cfg_options, scope, oc, file, line)
//ScenSim-Port//	int *ignorep;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct client_state *client_state;
//ScenSim-Port//	struct option_state *in_options;
...
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int evaluate_boolean_option_cache (int *ignorep, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct option_cache *oc, const char *file, int line)//ScenSim-Port//
{
...
//ScenSim-Port//int evaluate_boolean_expression_result (ignorep, packet, lease, client_state,
//ScenSim-Port//					in_options, cfg_options, scope, expr)
//ScenSim-Port//	int *ignorep;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
...
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct expression *expr;
int evaluate_boolean_expression_result (int *ignorep, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct expression *expr)//ScenSim-Port//
{
	int result;
...
//ScenSim-Port//void expression_dereference (eptr, file, line)
//ScenSim-Port//	struct expression **eptr;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
void expression_dereference (struct expression **eptr, const char *file, int line)//ScenSim-Port//
{
	...
	*eptr = (struct expression *)0;
	...
	switch (expr -> op) {
		...
		break;
	      ...
	      case expr_not:
//ScenSim-Port//		if (expr -> data.not)
//ScenSim-Port//			expression_dereference (&expr -> data.not, file, line);
		if (expr -> data.not_)//ScenSim-Port//
			expression_dereference (&expr -> data.not_, file, line);//ScenSim-Port//
		break;
...
//ScenSim-Port//int is_dns_expression (expr)
//ScenSim-Port//	struct expression *expr;
int is_dns_expression (struct expression *expr)//ScenSim-Port//
{
...
//ScenSim-Port//int is_boolean_expression (expr)
//ScenSim-Port//	struct expression *expr;
int is_boolean_expression (struct expression *expr)//ScenSim-Port//
{
...
//ScenSim-Port//int is_data_expression (expr)
//ScenSim-Port//	struct expression *expr;
int is_data_expression (struct expression *expr)//ScenSim-Port//
{
...
//ScenSim-Port//int is_numeric_expression (expr)
//ScenSim-Port//	struct expression *expr;
int is_numeric_expression (struct expression *expr)//ScenSim-Port//
{
...
//ScenSim-Port//int is_compound_expression (expr)
//ScenSim-Port//	struct expression *expr;
int is_compound_expression (struct expression *expr)//ScenSim-Port//
{
	return (expr -> op == expr_ns_add ||
		expr -> op == expr_ns_delete ||
		expr -> op == expr_ns_exists ||
...
//ScenSim-Port//static int op_val (op)
//ScenSim-Port//	enum expr_op op;
static int op_val (enum expr_op op)//ScenSim-Port//
{
	switch (op) {
	      case expr_none:
	      case expr_match:
...
//ScenSim-Port//int op_precedence (op1, op2)
//ScenSim-Port//	enum expr_op op1, op2;
int op_precedence (enum expr_op op1, enum expr_op op2)//ScenSim-Port//
{
...
//ScenSim-Port//enum expression_context op_context (op)
//ScenSim-Port//	enum expr_op op;
enum expression_context op_context (enum expr_op op)//ScenSim-Port//
{
	switch (op) {
...
//ScenSim-Port//int write_expression (file, expr, col, indent, firstp)
//ScenSim-Port//	FILE *file;
//ScenSim-Port//	struct expression *expr;
//ScenSim-Port//	int col;
//ScenSim-Port//	int indent;
//ScenSim-Port//	int firstp;
int write_expression (FILE *file, struct expression *expr, int col, int indent, int firstp)//ScenSim-Port//
{
	struct expression *e;
	const char *s;
	char obuf [65];
...
//ScenSim-Port//					expr -> data.not, col, indent + 2, 1);
					expr -> data.not_, col, indent + 2, 1);//ScenSim-Port//
		break;
...
//ScenSim-Port//int binding_scope_dereference (ptr, file, line)
//ScenSim-Port//	struct binding_scope **ptr;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int binding_scope_dereference (struct binding_scope **ptr, const char *file, int line)//ScenSim-Port//
{
...
//ScenSim-Port//int fundef_dereference (ptr, file, line)
//ScenSim-Port//	struct fundef **ptr;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int fundef_dereference (struct fundef **ptr, const char *file, int line)//ScenSim-Port//
{
...
//ScenSim-Port//	binding = find_binding (*scope, name);
	binding = (struct binding*)find_binding (*scope, name);//ScenSim-Port//
	if (!binding) {
//ScenSim-Port//		binding = dmalloc (sizeof *binding, MDL);
		binding = (struct binding*)dmalloc (sizeof *binding, MDL);//ScenSim-Port//
		if (!binding)
			return (struct binding *)0;

		memset (binding, 0, sizeof *binding);
//ScenSim-Port//		binding -> name = dmalloc (strlen (name) + 1, MDL);
		binding -> name = (char*)dmalloc (strlen (name) + 1, MDL);//ScenSim-Port//
		if (!binding -> name) {
			dfree (binding, MDL);
			return (struct binding *)0;
		}
