...
//ScenSim-Port//int execute_statements (result, packet, lease, client_state,
//ScenSim-Port//			in_options, out_options, scope, statements)
...
//ScenSim-Port//	struct option_state *out_options;
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct executable_statement *statements;
int execute_statements (struct binding_value **result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *out_options, struct binding_scope **scope, struct executable_statement *statements)//ScenSim-Port//
{
...
//ScenSim-Port//                        argv = dmalloc((argc + 2) * sizeof(*argv), MDL);
                        argv = (char**)dmalloc((argc + 2) * sizeof(*argv), MDL);//ScenSim-Port//
                        if (!argv)
                                break;

//ScenSim-Port//                        argv[0] = dmalloc(strlen(r->data.execute.command) + 1,
                        argv[0] = (char*)dmalloc(strlen(r->data.execute.command) + 1,//ScenSim-Port//
                                          MDL);
                        if (argv[0]) {
                        ...
                        for (i = 1, expr = r->data.execute.arglist; expr;
                             expr = expr->data.arg.next, i++) {
                                ...
                                if (status) {
//ScenSim-Port//                                        argv[i] = dmalloc(ds.len + 1, MDL);
                                        argv[i] = (char*)dmalloc(ds.len + 1, MDL);//ScenSim-Port//
                                        if (argv[i]) {
			...
			log_debug ("exec: add %s", (r -> data.add -> name
					       ...
					       : "<unnamed class>"));
#endif
//ScenSim-Port//			classify (packet, r -> data.add);
			curctx->classify (packet, r -> data.add);//ScenSim-Port//
			break;
...
//ScenSim-Port//				binding = dmalloc(sizeof(*binding), MDL);
				...
				if (binding != NULL) {
				    ...
				    binding->name =
//ScenSim-Port//					    dmalloc(strlen
					    (char*)dmalloc(strlen//ScenSim-Port//
						    (r->data.set.name) + 1,
						    MDL);
...
//ScenSim-Port//void execute_statements_in_scope (result, packet,
//ScenSim-Port//				  lease, client_state, in_options, out_options,
//ScenSim-Port//				  scope, group, limiting_group)
//ScenSim-Port//	struct binding_value **result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct client_state *client_state;
...
//ScenSim-Port//	struct group *limiting_group;
void execute_statements_in_scope (struct binding_value **result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *out_options, struct binding_scope **scope, struct group *group, struct group *limiting_group)//ScenSim-Port//
{
	struct group *limit;
...
//ScenSim-Port//int executable_statement_dereference (ptr, file, line)
//ScenSim-Port//	struct executable_statement **ptr;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int executable_statement_dereference (struct executable_statement **ptr, const char *file, int line)//ScenSim-Port//
{
	if (!ptr || !*ptr) {
...
//ScenSim-Port//void write_statements (file, statements, indent)
//ScenSim-Port//	FILE *file;
//ScenSim-Port//	struct executable_statement *statements;
//ScenSim-Port//	int indent;
void write_statements (FILE *file, struct executable_statement *statements, int indent)//ScenSim-Port//
{
