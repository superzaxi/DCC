...
//ScenSim-Port//int db_time_format = DEFAULT_TIME_FORMAT;
...
//ScenSim-Port//	buf = dmalloc (len + 1, file, line);
	buf = (char*)dmalloc (len + 1, file, line);//ScenSim-Port//
	if (buf) {
		nsp = buf;
		for (sp = s; sp && *sp; sp++) {
			if (*sp == ' ')
...
//ScenSim-Port//	buf = dmalloc (nulen + 1, MDL);
	buf = (char*)dmalloc (nulen + 1, MDL);//ScenSim-Port//
	if (buf) {
		nsp = buf;
		for (i = 0; i < len; i++) {
			if (s [i] == ' ')
		    ...
		    const char *file, int line)
{
	...
	bl = ((len * 4 + 2) / 3) + 1;
//ScenSim-Port//	b = dmalloc (bl + 1, file, line);
	b = (char*)dmalloc (bl + 1, file, line);//ScenSim-Port//
	if (!b)
		return (char *)0;
...
//ScenSim-Port//char *print_hw_addr (htype, hlen, data)
//ScenSim-Port//	const int htype;
//ScenSim-Port//	const int hlen;
//ScenSim-Port//	const unsigned char *data;
char *print_hw_addr (const int htype, const int hlen, const unsigned char *data)//ScenSim-Port//
{
...
//ScenSim-Port//void print_lease (lease)
//ScenSim-Port//	struct lease *lease;
void print_lease (struct lease *lease)//ScenSim-Port//
{
	struct tm *t;
	char tbuf [32];
...
//ScenSim-Port//void dump_packet (tp)
//ScenSim-Port//	struct packet *tp;
void dump_packet (struct packet *tp)//ScenSim-Port//
{
...
//ScenSim-Port//void dump_raw (buf, len)
//ScenSim-Port//	const unsigned char *buf;
//ScenSim-Port//	unsigned len;
void dump_raw (const unsigned char *buf, unsigned len)//ScenSim-Port//
{
	int i;
	char lbuf [80];
	int lbix = 0;
...
//ScenSim-Port//void hash_dump (table)
//ScenSim-Port//	struct hash_table *table;
void hash_dump (struct hash_table *table)//ScenSim-Port//
{
	int i;
 ...
 * buf - output buffer
 */
//ScenSim-Port//void print_hex_only (len, data, limit, buf)
//ScenSim-Port//	unsigned len;
//ScenSim-Port//	const u_int8_t *data;
//ScenSim-Port//	unsigned limit;
//ScenSim-Port//	char *buf;
void print_hex_only (unsigned len, const u_int8_t *data, unsigned limit, char *buf)//ScenSim-Port//
{
	unsigned i;
 ...
 * buf - output buffer
 */
//ScenSim-Port//void print_hex_or_string (len, data, limit, buf)
//ScenSim-Port//	unsigned len;
//ScenSim-Port//	const u_int8_t *data;
//ScenSim-Port//	unsigned limit;
//ScenSim-Port//	char *buf;
void print_hex_or_string (unsigned len, const u_int8_t *data, unsigned limit, char *buf)//ScenSim-Port//
{
	unsigned i;
	if ((buf == NULL) || (limit < 3))
		return;
 ...
 */
#define HBLEN 1024
//ScenSim-Port//char *print_hex(len, data, limit, buf_num)
//ScenSim-Port//	unsigned len;
//ScenSim-Port//	const u_int8_t *data;
//ScenSim-Port//	unsigned limit;
//ScenSim-Port//	unsigned buf_num;
char *print_hex(unsigned len, const u_int8_t *data, unsigned limit, unsigned buf_num)//ScenSim-Port//
{
...
//ScenSim-Port//char *print_dotted_quads (len, data)
//ScenSim-Port//	unsigned len;
//ScenSim-Port//	const u_int8_t *data;
char *print_dotted_quads (unsigned len, const u_int8_t *data)//ScenSim-Port//
{
...
//ScenSim-Port//char *print_dec_1 (val)
//ScenSim-Port//	unsigned long val;
char *print_dec_1 (unsigned long val)//ScenSim-Port//
{
	static char vbuf [32];
	sprintf (vbuf, "%lu", val);
	return vbuf;
...
//ScenSim-Port//char *print_dec_2 (val)
//ScenSim-Port//	unsigned long val;
char *print_dec_2 (unsigned long val)//ScenSim-Port//
{
	static char vbuf [32];
	sprintf (vbuf, "%lu", val);
	return vbuf;
...
//ScenSim-Port//static unsigned print_subexpression (expr, buf, len)
//ScenSim-Port//	struct expression *expr;
//ScenSim-Port//	char *buf;
//ScenSim-Port//	unsigned len;
static unsigned print_subexpression (struct expression *expr, char *buf, unsigned len)//ScenSim-Port//
{
	unsigned rv, left;
	const char *s;
	...
	switch (expr -> op) {
		...
		if (len > rv + 4) {
			...
			rv += 1;
			buf [rv++] = ' ';
//ScenSim-Port//			rv += print_subexpression (expr -> data.and [0],
			rv += print_subexpression (expr -> data.and_ [0],//ScenSim-Port//
						buf + rv, len - rv - 2);
			buf [rv++] = ' ';
//ScenSim-Port//			rv += print_subexpression (expr -> data.and [1],
			rv += print_subexpression (expr -> data.and_ [1],//ScenSim-Port//
						   buf + rv, len - rv - 1);
			buf [rv++] = ')';
			buf [rv] = 0;
			return rv;
		...
		if (len > 6) {
			rv = 5;
			strcpy (buf, "(not ");
//ScenSim-Port//			rv += print_subexpression (expr -> data.not,
			rv += print_subexpression (expr -> data.not_,//ScenSim-Port//
						   buf + rv, len - rv - 1);
			buf [rv++] = ')';
			buf [rv] = 0;
			return rv;
...
//ScenSim-Port//void print_expression (name, expr)
//ScenSim-Port//	const char *name;
//ScenSim-Port//	struct expression *expr;
void print_expression (const char *name, struct expression *expr)//ScenSim-Port//
{
	char buf [1024];
...
//ScenSim-Port//	t = dmalloc (len + 1, MDL);
	t = (char*)dmalloc (len + 1, MDL);//ScenSim-Port//
	if (!t)
			      ...
			      struct data_string *data)
{
	...
	if (i == data -> len) {
//ScenSim-Port//		buf = dmalloc (data -> len + 3, MDL);
		buf = (char*)dmalloc (data -> len + 3, MDL);//ScenSim-Port//
		if (buf) {
			buf [0] = '"';
