...
//ScenSim-Port//#define DHCP_OPTION_DATA
//ScenSim-Port//#include "dhcpd.h"
//ScenSim-Port//#include <omapip/omapip_p.h>
//ScenSim-Port//#include <limits.h>
#define new new_str//ScenSim-Port//

//ScenSim-Port//struct option *vendor_cfg_option;
...
//ScenSim-Port//int parse_options (packet)
//ScenSim-Port//	struct packet *packet;
int parse_options (struct packet *packet)//ScenSim-Port//
{
 ...
 */
//ScenSim-Port//int parse_option_buffer (options, buffer, length, universe)
//ScenSim-Port//	struct option_state *options;
//ScenSim-Port//	const unsigned char *buffer;
//ScenSim-Port//	unsigned length;
//ScenSim-Port//	struct universe *universe;
int parse_option_buffer (struct option_state *options, const unsigned char *buffer, unsigned length, struct universe *universe)//ScenSim-Port//
{
	unsigned len, offset;
	unsigned code;
...
//ScenSim-Port//	char *reason = "general failure";
	const char *reason = "general failure";//ScenSim-Port//
...
//ScenSim-Port//	s = strchr (eopt -> format, 'E');
	s = strchr ((char*)eopt -> format, 'E');//ScenSim-Port//
	if (!s) {
	...
	if (t == s && uname) {
//ScenSim-Port//		for (i = 0; i < universe_count; i++) {
		for (i = 0; i < universe_count_; i++) {//ScenSim-Port//
//ScenSim-Port//			if (!strcmp (universes [i] -> name, uname)) {
			if (!strcmp (universes_ [i] -> name, uname)) {//ScenSim-Port//
//ScenSim-Port//				universe = universes [i];
				universe = universes_ [i];//ScenSim-Port//
				break;
			}
		}
	} else if (t != s) {
//ScenSim-Port//		for (i = 0; i < universe_count; i++) {
		for (i = 0; i < universe_count_; i++) {//ScenSim-Port//
//ScenSim-Port//			if (strlen (universes [i] -> name) == t - s &&
			if (strlen (universes_ [i] -> name) == t - s &&//ScenSim-Port//
//ScenSim-Port//			    !memcmp (universes [i] -> name,
			    !memcmp (universes_ [i] -> name,//ScenSim-Port//
				     s, (unsigned)(t - s))) {
//ScenSim-Port//				universe = universes [i];
				universe = universes_ [i];//ScenSim-Port//
				break;
...
//ScenSim-Port//			hash = cfg_options->universes[dhcp_universe.index];
			hash = (_pair**)cfg_options->universes[dhcp_universe.index];//ScenSim-Port//
			if (hash) {
		 ...
		 */
		for (i = 0; i < OPTION_HASH_SIZE; i++) {
//ScenSim-Port//		    hash = cfg_options->universes[cfg_options->site_universe];
		    hash = (_pair**)cfg_options->universes[cfg_options->site_universe];//ScenSim-Port//
		    if (hash != NULL)
			for (pp = hash[i]; pp; pp = pp->cdr) {
				op = (struct option_cache *)(pp->car);
				if (op->option->code >=
...
//ScenSim-Port//		    if (universes[i]->enc_opt &&
		    if (universes_[i]->enc_opt &&//ScenSim-Port//
			priority_len < PRIORITY_COUNT &&
//ScenSim-Port//			universes[i]->enc_opt->universe == &dhcp_universe) {
			universes_[i]->enc_opt->universe == &dhcp_universe) {//ScenSim-Port//
//ScenSim-Port//			    if (universes[i]->enc_opt->code !=
			    if (universes_[i]->enc_opt->code !=//ScenSim-Port//
				DHO_DHCP_AGENT_OPTIONS)
				    priority_list[priority_len++] =
//ScenSim-Port//					    universes[i]->enc_opt->code;
					    universes_[i]->enc_opt->code;//ScenSim-Port//
		    }
		}
...
static void
//ScenSim-Port//vsio_options(struct option_cache *oc,
vsio_options_(struct option_cache *oc,//ScenSim-Port//
...
//ScenSim-Port//		    		o = universes[i]->enc_opt;
		    		o = universes_[i]->enc_opt;//ScenSim-Port//
				if ((o != NULL) && 
...
//ScenSim-Port//							     universes[i], 
							     ...
							     (void *)&vs,
//ScenSim-Port//			     				     vsio_options);
			     				     vsio_options_);//ScenSim-Port//
...
//ScenSim-Port//		    u = universes [cfg_options -> site_universe];
		    u = universes_ [cfg_options -> site_universe];//ScenSim-Port//
	    else
		    u = &dhcp_universe;
...
//ScenSim-Port//		s = strchr (option->format, 'E');
		s = strchr ((char*)option->format, 'E');//ScenSim-Port//
		if (s)
		    t = strchr (++s, '.');
		if (s && t) {
...
int
//ScenSim-Port//format_has_text(format)
//ScenSim-Port//	const char *format;
format_has_text(const char *format)//ScenSim-Port//
{
	const char *p;
...
int
//ScenSim-Port//format_min_length(format, oc)
//ScenSim-Port//	const char *format;
//ScenSim-Port//	struct option_cache *oc;
format_min_length(const char *format, struct option_cache *oc)//ScenSim-Port//
{
	const char *p, *name;
	int min_len = 0;
	int last_size = 0;
...
//ScenSim-Port//const char *pretty_print_option (option, data, len, emit_commas, emit_quotes)
//ScenSim-Port//	struct option *option;
//ScenSim-Port//	const unsigned char *data;
//ScenSim-Port//	unsigned len;
//ScenSim-Port//	int emit_commas;
//ScenSim-Port//	int emit_quotes;
const char *pretty_print_option (struct option *option, const unsigned char *data, unsigned len, int emit_commas, int emit_quotes)//ScenSim-Port//
{
...
//ScenSim-Port//int get_option (result, universe, packet, lease, client_state,
//ScenSim-Port//		in_options, cfg_options, options, scope, code, file, line)
...
//ScenSim-Port//	unsigned code;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int get_option (struct data_string *result, struct universe *universe, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct option_state *options, struct binding_scope **scope, unsigned code, const char *file, int line)//ScenSim-Port//
{
	struct option_cache *oc;
...
//ScenSim-Port//void set_option (universe, options, option, op)
//ScenSim-Port//	struct universe *universe;
//ScenSim-Port//	struct option_state *options;
//ScenSim-Port//	struct option_cache *option;
//ScenSim-Port//	enum statement_op op;
void set_option (struct universe *universe, struct option_state *options, struct option_cache *option, enum statement_op op)//ScenSim-Port//
{
...
//ScenSim-Port//struct option_cache *lookup_option (universe, options, code)
//ScenSim-Port//	struct universe *universe;
//ScenSim-Port//	struct option_state *options;
//ScenSim-Port//	unsigned code;
struct option_cache *lookup_option (struct universe *universe, struct option_state *options, unsigned code)//ScenSim-Port//
{
	if (!options)
...
//ScenSim-Port//struct option_cache *lookup_hashed_option (universe, options, code)
//ScenSim-Port//	struct universe *universe;
//ScenSim-Port//	struct option_state *options;
//ScenSim-Port//	unsigned code;
struct option_cache *lookup_hashed_option (struct universe *universe, struct option_state *options, unsigned code)//ScenSim-Port//
{
	int hashix;
	pair bptr;
	pair *hash;
...
//ScenSim-Port//	hash = options -> universes [universe -> index];
	hash = (_pair**)options -> universes [universe -> index];//ScenSim-Port//
		   ...
		   struct option_cache *oc, isc_boolean_t appendp)
{
	int hashix;
	pair bptr;
//ScenSim-Port//	pair *hash = options -> universes [universe -> index];
	pair *hash = (_pair**)options -> universes [universe -> index];//ScenSim-Port//
...
//ScenSim-Port//void delete_option (universe, options, code)
//ScenSim-Port//	struct universe *universe;
//ScenSim-Port//	struct option_state *options;
//ScenSim-Port//	int code;
void delete_option (struct universe *universe, struct option_state *options, int code)//ScenSim-Port//
{
...
//ScenSim-Port//void delete_hashed_option (universe, options, code)
//ScenSim-Port//	struct universe *universe;
//ScenSim-Port//	struct option_state *options;
//ScenSim-Port//	int code;
void delete_hashed_option (struct universe *universe, struct option_state *options, int code)//ScenSim-Port//
{
	int hashix;
...
//ScenSim-Port//int option_cache_dereference (ptr, file, line)
//ScenSim-Port//	struct option_cache **ptr;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int option_cache_dereference (struct option_cache **ptr, const char *file, int line)//ScenSim-Port//
{
	if (!ptr || !*ptr) {
...
//ScenSim-Port//int hashed_option_state_dereference (universe, state, file, line)
//ScenSim-Port//	struct universe *universe;
//ScenSim-Port//	struct option_state *state;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int hashed_option_state_dereference (struct universe *universe, struct option_state *state, const char *file, int line)//ScenSim-Port//
{
	pair *heads;
	pair cp, next;
	int i;
...
//ScenSim-Port//				start = strchr(oc->option->format, 'E');
				start = strchr((char*)oc->option->format, 'E');//ScenSim-Port//
				if (start == NULL)
					break;
...
//ScenSim-Port//int option_space_encapsulate (result, packet, lease, client_state,
//ScenSim-Port//			      in_options, cfg_options, scope, name)
//ScenSim-Port//	struct data_string *result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct client_state *client_state;
...
int option_space_encapsulate (struct data_string *result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct data_string *name)//ScenSim-Port//
{
			...
			struct universe *universe)
{
	...
	for (i = 0 ; i < cfg_options->universe_count ; i++) {
//ScenSim-Port//		subu = universes[i];
		subu = universes_[i];//ScenSim-Port//
...
//ScenSim-Port//int hashed_option_space_encapsulate (result, packet, lease, client_state,
//ScenSim-Port//				     in_options, cfg_options, scope, universe)
//ScenSim-Port//	struct data_string *result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct client_state *client_state;
...
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct universe *universe;
int hashed_option_space_encapsulate (struct data_string *result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct universe *universe)//ScenSim-Port//
{
	pair p, *hash;
	int status;
	int i;
...
//ScenSim-Port//	hash = cfg_options -> universes [universe -> index];
	hash = (_pair**)cfg_options -> universes [universe -> index];//ScenSim-Port//
	if (!hash)
		return 0;
...
//ScenSim-Port//int nwip_option_space_encapsulate (result, packet, lease, client_state,
//ScenSim-Port//				   in_options, cfg_options, scope, universe)
//ScenSim-Port//	struct data_string *result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
...
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct universe *universe;
int nwip_option_space_encapsulate (struct data_string *result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct universe *universe)//ScenSim-Port//
{
	pair ocp;
	int status;
...
//ScenSim-Port//int fqdn_option_space_encapsulate (result, packet, lease, client_state,
//ScenSim-Port//				   in_options, cfg_options, scope, universe)
//ScenSim-Port//	struct data_string *result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
...
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct universe *universe;
int fqdn_option_space_encapsulate (struct data_string *result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct universe *universe)//ScenSim-Port//
{
	pair ocp;
...
//ScenSim-Port//	hash = cfg_options -> universes [u -> index];
	hash = (_pair**)cfg_options -> universes [u -> index];//ScenSim-Port//
	if (!hash)
		return;
	for (i = 0; i < OPTION_HASH_SIZE; i++) {
		pair p;
...
//ScenSim-Port//int linked_option_space_encapsulate (result, packet, lease, client_state,
//ScenSim-Port//				    in_options, cfg_options, scope, universe)
//ScenSim-Port//	struct data_string *result;
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
...
//ScenSim-Port//	struct binding_scope **scope;
//ScenSim-Port//	struct universe *universe;
int linked_option_space_encapsulate (struct data_string *result, struct packet *packet, struct lease *lease, struct client_state *client_state, struct option_state *in_options, struct option_state *cfg_options, struct binding_scope **scope, struct universe *universe)//ScenSim-Port//
{
	int status = 0;
	pair oc;
	struct option_chain_head *head;
...
//ScenSim-Port//void delete_linked_option (universe, options, code)
//ScenSim-Port//	struct universe *universe;
//ScenSim-Port//	struct option_state *options;
//ScenSim-Port//	int code;
void delete_linked_option (struct universe *universe, struct option_state *options, int code)//ScenSim-Port//
{
...
//ScenSim-Port//struct option_cache *lookup_linked_option (universe, options, code)
//ScenSim-Port//	struct universe *universe;
//ScenSim-Port//	struct option_state *options;
//ScenSim-Port//	unsigned code;
struct option_cache *lookup_linked_option (struct universe *universe, struct option_state *options, unsigned code)//ScenSim-Port//
{
	pair oc;
	struct option_chain_head *head;
...
//ScenSim-Port//int linked_option_state_dereference (universe, state, file, line)
//ScenSim-Port//	struct universe *universe;
//ScenSim-Port//	struct option_state *state;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int linked_option_state_dereference (struct universe *universe, struct option_state *state, const char *file, int line)//ScenSim-Port//
{
...
//ScenSim-Port//void do_packet (interface, packet, len, from_port, from, hfrom)
//ScenSim-Port//	struct interface_info *interface;
//ScenSim-Port//	struct dhcp_packet *packet;
//ScenSim-Port//	unsigned len;
//ScenSim-Port//	unsigned int from_port;
//ScenSim-Port//	struct iaddr from;
//ScenSim-Port//	struct hardware *hfrom;
void do_packet (struct interface_info *interface, struct dhcp_packet *packet, unsigned len, unsigned int from_port, struct iaddr from, struct hardware *hfrom)//ScenSim-Port//
{
	struct option_cache *op;
...
//ScenSim-Port//			dhcp(decoded_packet);
			curctx->dhcp(decoded_packet);//ScenSim-Port//
		else
//ScenSim-Port//			bootp(decoded_packet);
			curctx->bootp(decoded_packet);//ScenSim-Port//
	}
...
//ScenSim-Port//	dhcpv6(decoded_packet);
	curctx->dhcpv6(decoded_packet);//ScenSim-Port//
...
//ScenSim-Port//			     data, 
			     (const unsigned char*)data, //ScenSim-Port//
			     data_len,
			     0, 
			     0, 
			     MDL)) {
