...
//ScenSim-Port//struct iaddr subnet_number (addr, mask)
//ScenSim-Port//	struct iaddr addr;
//ScenSim-Port//	struct iaddr mask;
struct iaddr subnet_number (struct iaddr addr, struct iaddr mask)//ScenSim-Port//
{
	int i;
	struct iaddr rv;
...
//ScenSim-Port//struct iaddr ip_addr (subnet, mask, host_address)
//ScenSim-Port//	struct iaddr subnet;
//ScenSim-Port//	struct iaddr mask;
//ScenSim-Port//	u_int32_t host_address;
struct iaddr ip_addr (struct iaddr subnet, struct iaddr mask, u_int32_t host_address)//ScenSim-Port//
{
	int i, j, k;
	u_int32_t swaddr;
	struct iaddr rv;
...
//ScenSim-Port//struct iaddr broadcast_addr (subnet, mask)
//ScenSim-Port//	struct iaddr subnet;
//ScenSim-Port//	struct iaddr mask;
struct iaddr broadcast_addr (struct iaddr subnet, struct iaddr mask)//ScenSim-Port//
{
	int i;
	struct iaddr rv;
...
//ScenSim-Port//u_int32_t host_addr (addr, mask)
//ScenSim-Port//	struct iaddr addr;
//ScenSim-Port//	struct iaddr mask;
u_int32_t host_addr (struct iaddr addr, struct iaddr mask)//ScenSim-Port//
{
	int i;
	u_int32_t swaddr;
	struct iaddr rv;
...
//ScenSim-Port//int addr_eq (addr1, addr2)
//ScenSim-Port//	struct iaddr addr1, addr2;
int addr_eq (struct iaddr addr1, struct iaddr addr2)//ScenSim-Port//
{
 ...
 */
int
//ScenSim-Port//addr_match(addr, match)
//ScenSim-Port//	struct iaddr *addr;
//ScenSim-Port//	struct iaddrmatch *match;
addr_match(struct iaddr *addr, struct iaddrmatch *match)//ScenSim-Port//
{
        int i;
...
//ScenSim-Port//	return (((byte >> shift_bits) << shift_bits) == byte);
	return (isc_boolean_t)(((byte >> shift_bits) << shift_bits) == byte);//ScenSim-Port//
}
...
range2cidr(struct iaddrcidrnetlist **result, 
	...
	while (addr_cmp(&addr, hi) <= 0) {
		...
		if ((ofs < 0) ||
			 ...
			 */
//ScenSim-Port//			net = dmalloc(sizeof(*net), MDL);
			net = (iaddrcidrnetlist*)dmalloc(sizeof(*net), MDL);//ScenSim-Port//
			if (net == NULL) {
				while (*result != NULL) {
					net = (*result)->next;
					dfree(*result, MDL);
