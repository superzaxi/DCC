...
//ScenSim-Port//#ifndef lint
//ScenSim-Port//static const char rcsid[] = "$Id: ns_name.c,v 1.2 2009/10/28 04:12:29 sar Exp $";
//ScenSim-Port//#endif
...
MRns_name_pton(const char *src, u_char *dst, size_t dstsiz) {
	...
	while ((c = *src++) != 0) {
		if (escaped) {
//ScenSim-Port//			if ((cp = strchr(digits, c)) != NULL) {
			if ((cp = strchr((char*)digits, c)) != NULL) {//ScenSim-Port//
				n = (cp - digits) * 100;
				if ((c = *src++) == 0 ||
//ScenSim-Port//				    (cp = strchr(digits, c)) == NULL) {
				    (cp = strchr((char*)digits, c)) == NULL) {//ScenSim-Port//
					errno = EMSGSIZE;
					return (-1);
				}
				n += (cp - digits) * 10;
