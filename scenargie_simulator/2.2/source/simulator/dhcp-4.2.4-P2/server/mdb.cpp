...
//ScenSim-Port//#include "omapip/hash.h"

//ScenSim-Port//struct subnet *subnets;
#define subnets SCENSIMGLOBAL(subnets)//ScenSim-Port//
//ScenSim-Port//struct shared_network *shared_networks;
//ScenSim-Port//host_hash_t *host_hw_addr_hash;
//ScenSim-Port//host_hash_t *host_uid_hash;
//ScenSim-Port//host_hash_t *host_name_hash;
//ScenSim-Port//lease_id_hash_t *lease_uid_hash;
//ScenSim-Port//lease_ip_hash_t *lease_ip_addr_hash;
//ScenSim-Port//lease_id_hash_t *lease_hw_addr_hash;
 ...
 */
//ScenSim-Port//typedef struct host_id_info {
//ScenSim-Port//	struct option *option;
//ScenSim-Port//	host_hash_t *values_hash;
//ScenSim-Port//	struct host_id_info *next;
//ScenSim-Port//} host_id_info_t;

//ScenSim-Port//static host_id_info_t *host_id_info = NULL;

//ScenSim-Port//int numclasseswritten;

//ScenSim-Port//omapi_object_type_t *dhcp_type_host;

//ScenSim-Port//isc_result_t enter_class(cd, dynamicp, commit)
//ScenSim-Port//	struct class *cd;
//ScenSim-Port//	int dynamicp;
//ScenSim-Port//	int commit;
isc_result_t enter_class(struct class_type *cd, int dynamicp, int commit)//ScenSim-Port//
{
...
//ScenSim-Port//		struct class *c = 0;
		struct class_type *c = 0;//ScenSim-Port//
...
static int find_uid_statement (struct executable_statement *esp,
			       void *vp, int condp)
{
//ScenSim-Port//	struct executable_statement **evp = vp;
	struct executable_statement **evp = (struct executable_statement**)vp;//ScenSim-Port//
...
//ScenSim-Port//isc_result_t enter_host (hd, dynamicp, commit)
//ScenSim-Port//	struct host_decl *hd;
//ScenSim-Port//	int dynamicp;
//ScenSim-Port//	int commit;
isc_result_t enter_host (struct host_decl *hd, int dynamicp, int commit)//ScenSim-Port//
{
	...
	if (hd->host_id_option != NULL) {
		...
		if (h_id_info == NULL) {
//ScenSim-Port//			h_id_info = dmalloc(sizeof(*h_id_info), MDL);
			h_id_info = (host_id_info_t*)dmalloc(sizeof(*h_id_info), MDL);//ScenSim-Port//
			if (h_id_info == NULL) {
...
//ScenSim-Port//isc_result_t delete_class (cp, commit)
//ScenSim-Port//	struct class *cp;
//ScenSim-Port//	int commit;
isc_result_t delete_class (struct class_type *cp, int commit)//ScenSim-Port//
{
...
//ScenSim-Port//isc_result_t delete_host (hd, commit)
//ScenSim-Port//	struct host_decl *hd;
//ScenSim-Port//	int commit;
isc_result_t delete_host (struct host_decl *hd, int commit)//ScenSim-Port//
{
	struct host_decl *hp = (struct host_decl *)0;
	struct host_decl *np = (struct host_decl *)0;
...
//ScenSim-Port//void new_address_range (cfile, low, high, subnet, pool, lpchain)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct iaddr low, high;
//ScenSim-Port//	struct subnet *subnet;
//ScenSim-Port//	struct pool *pool;
//ScenSim-Port//	struct lease **lpchain;
void new_address_range (struct parse *cfile, struct iaddr low, struct iaddr high, struct subnet *subnet, struct pool *pool, struct lease **lpchain)//ScenSim-Port//
{
...
//ScenSim-Port//			log_fatal ("No memory for lease %s: %s",
			log_fatal ("No memory for lease %s: %i",//ScenSim-Port//
...
//ScenSim-Port//void enter_subnet (subnet)
//ScenSim-Port//	struct subnet *subnet;
void enter_subnet (struct subnet *subnet)//ScenSim-Port//
{
...
//ScenSim-Port//void enter_shared_network (share)
//ScenSim-Port//	struct shared_network *share;
void enter_shared_network (struct shared_network *share)//ScenSim-Port//
{
	if (shared_networks) {
...
//ScenSim-Port//void new_shared_network_interface (cfile, share, name)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct shared_network *share;
//ScenSim-Port//	const char *name;
void new_shared_network_interface (struct parse *cfile, struct shared_network *share, const char *name)//ScenSim-Port//
{
	...
	if (!ip) {
		...
		if (status != ISC_R_SUCCESS)
//ScenSim-Port//			log_fatal ("new_shared_network_interface %s: %s",
//ScenSim-Port//				   name, isc_result_totext (status));
			log_fatal ("new_shared_network_interface %s: %i", name, status);//ScenSim-Port//
...
//ScenSim-Port//void enter_lease (lease)
//ScenSim-Port//	struct lease *lease;
void enter_lease (struct lease *lease)//ScenSim-Port//
{
...
//ScenSim-Port//int supersede_lease (comp, lease, commit, propogate, pimmediate)
//ScenSim-Port//	struct lease *comp, *lease;
//ScenSim-Port//	int commit;
//ScenSim-Port//	int propogate;
//ScenSim-Port//	int pimmediate;
int supersede_lease (struct lease *comp, struct lease *lease, int commit, int propogate, int pimmediate)//ScenSim-Port//
{
...
//ScenSim-Port//		lt -> uid = dmalloc (lt -> uid_max, MDL);
		lt -> uid = (unsigned char*)dmalloc (lt -> uid_max, MDL);//ScenSim-Port//
		if (!lt -> uid) {
			lease_dereference (&lt, MDL);
			return 0;
		}
	...
	if (lease -> client_hostname) {
		lt -> client_hostname =
//ScenSim-Port//			dmalloc (strlen (lease -> client_hostname) + 1, MDL);
			(char*)dmalloc (strlen (lease -> client_hostname) + 1, MDL);//ScenSim-Port//
...
//ScenSim-Port//void release_lease (lease, packet)
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct packet *packet;
void release_lease (struct lease *lease, struct packet *packet)//ScenSim-Port//
{
...
//ScenSim-Port//void abandon_lease (lease, message)
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	const char *message;
void abandon_lease (struct lease *lease, const char *message)//ScenSim-Port//
{
	struct lease *lt = (struct lease *)0;
#if defined (NSUPDATE)
	(void) ddns_removals(lease, NULL, NULL, ISC_FALSE);
...
//ScenSim-Port//void pool_timer (vpool)
//ScenSim-Port//	void *vpool;
void pool_timer (void *vpool)//ScenSim-Port//
{
	struct pool *pool;
...
//ScenSim-Port//void uid_hash_delete (lease)
//ScenSim-Port//	struct lease *lease;
void uid_hash_delete (struct lease *lease)//ScenSim-Port//
{
...
//ScenSim-Port//void hw_hash_delete (lease)
//ScenSim-Port//	struct lease *lease;
void hw_hash_delete (struct lease *lease)//ScenSim-Port//
{
	struct lease *head = (struct lease *)0;
	struct lease *next = (struct lease *)0;
...
//ScenSim-Port//	struct class *cp;
	struct class_type *cp;//ScenSim-Port//
...
#define class class_local//ScenSim-Port//
isc_result_t
lease_instantiate(const void *key, unsigned len, void *object)
{
//ScenSim-Port//	struct lease *lease = object;
	struct lease *lease = (struct lease*)object;//ScenSim-Port//
//ScenSim-Port//	struct class *class;
	struct class_type *class;//ScenSim-Port//
	...
	if (lease -> billing_class) {
//ScenSim-Port//		class = (struct class *)0;
		class = (struct class_type *)0;//ScenSim-Port//
...
//ScenSim-Port//HASH_FUNCTIONS (class, const char *, struct class, class_hash_t,
//ScenSim-Port//		class_reference, class_dereference, do_string_hash)
HASH_FUNCTIONS (class, const char *, struct class_type, class_hash_t,
...
//ScenSim-Port//extern struct hash_table *dns_zone_hash;
extern struct interface_info **interface_vector;
extern int interface_count;
//ScenSim-Port//dhcp_control_object_t *dhcp_control_object;
//ScenSim-Port//extern struct hash_table *auth_key_hash;
//ScenSim-Port//struct hash_table *universe_hash;
//ScenSim-Port//struct universe **universes;
//ScenSim-Port//int universe_count, universe_max;
#if 0
extern int end;
#endif
...
//ScenSim-Port//	struct class *cc = (struct class *)0, *cn = (struct class *)0;
	struct class_type *cc = (struct class_type *)0, *cn = (struct class_type *)0;//ScenSim-Port//
	...
	host_name_hash = 0;
//ScenSim-Port//	if (dns_zone_hash)
//ScenSim-Port//		dns_zone_free_hash_table (&dns_zone_hash, MDL);
//ScenSim-Port//	dns_zone_hash = 0;
...
//ScenSim-Port//	cancel_all_timeouts ();
//ScenSim-Port//	relinquish_timeouts ();
	relinquish_ackqueue();
//ScenSim-Port//	trace_free_all ();
...
//ScenSim-Port//	omapi_object_dereference ((omapi_object_t **)&icmp_state, MDL);
...
#endif
//ScenSim-Port//		if (universes [i]) {
		if (universes_ [i]) {//ScenSim-Port//
//ScenSim-Port//			if (universes[i]->name_hash)
			if (universes_[i]->name_hash)//ScenSim-Port//
			    option_name_free_hash_table(
//ScenSim-Port//						&universes[i]->name_hash,
						&universes_[i]->name_hash,//ScenSim-Port//
						MDL);
//ScenSim-Port//			if (universes[i]->code_hash)
			if (universes_[i]->code_hash)//ScenSim-Port//
			    option_code_free_hash_table(
//ScenSim-Port//						&universes[i]->code_hash,
						&universes_[i]->code_hash,//ScenSim-Port//
						MDL);
#if 0
...
#endif
//ScenSim-Port//	relinquish_hash_bucket_hunks ();
	omapi_type_relinquish ();
}
//ScenSim-Port//#endif /* DEBUG_MEMORY_LEAKAGE_ON_EXIT */
}//namespace IscDhcpPort////ScenSim-Port//
