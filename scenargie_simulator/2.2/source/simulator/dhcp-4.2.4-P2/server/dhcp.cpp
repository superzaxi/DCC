...
//ScenSim-Port//int outstanding_pings;

//ScenSim-Port//struct leasequeue *ackqueue_head, *ackqueue_tail;
//ScenSim-Port//static struct leasequeue *free_ackqueue;
//ScenSim-Port//static struct timeval max_fsync;

//ScenSim-Port//int outstanding_acks;
//ScenSim-Port//int max_outstanding_acks = DEFAULT_DELAYED_ACK;
//ScenSim-Port//int max_ack_delay_secs = DEFAULT_ACK_DELAY_SECS;
//ScenSim-Port//int max_ack_delay_usecs = DEFAULT_ACK_DELAY_USECS;
//ScenSim-Port//int min_ack_delay_usecs = DEFAULT_MIN_ACK_DELAY_USECS;

static char dhcp_message [256];
//ScenSim-Port//static int site_code_min;
...
//ScenSim-Port//void dhcpdiscover (packet, ms_nulltp)
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	int ms_nulltp;
void dhcpdiscover (struct packet *packet, int ms_nulltp)//ScenSim-Port//
{
...
//ScenSim-Port//void dhcprequest (packet, ms_nulltp, ip_lease)
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	int ms_nulltp;
//ScenSim-Port//	struct lease *ip_lease;
void dhcprequest (struct packet *packet, int ms_nulltp, struct lease *ip_lease)//ScenSim-Port//
{
	struct lease *lease;
	struct iaddr cip;
	struct iaddr sip;
...
//ScenSim-Port//void dhcprelease (packet, ms_nulltp)
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	int ms_nulltp;
void dhcprelease (struct packet *packet, int ms_nulltp)//ScenSim-Port//
{
...
//ScenSim-Port//void dhcpdecline (packet, ms_nulltp)
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	int ms_nulltp;
void dhcpdecline (struct packet *packet, int ms_nulltp)//ScenSim-Port//
{
	struct lease *lease = (struct lease *)0;
...
//ScenSim-Port//void dhcpinform (packet, ms_nulltp)
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	int ms_nulltp;
void dhcpinform (struct packet *packet, int ms_nulltp)//ScenSim-Port//
{
	char msgbuf [1024];
...
//ScenSim-Port//void nak_lease (packet, cip)
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct iaddr *cip;
void nak_lease (struct packet *packet, struct iaddr *cip)//ScenSim-Port//
{
	struct sockaddr_in to;
	struct in_addr from;
	int result;
...
//ScenSim-Port//void ack_lease (packet, lease, offer, when, msg, ms_nulltp, hp)
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	unsigned int offer;
//ScenSim-Port//	TIME when;
//ScenSim-Port//	char *msg;
//ScenSim-Port//	int ms_nulltp;
//ScenSim-Port//	struct host_decl *hp;
void ack_lease (struct packet *packet, struct lease *lease, unsigned int offer, TIME when, char *msg, int ms_nulltp, struct host_decl *hp)//ScenSim-Port//
{
	struct lease *lt;
...
//ScenSim-Port//		log_info ("%s: can't allocate temporary lease structure: %s",
//ScenSim-Port//			  msg, isc_result_totext (result));
		log_info ("%s: can't allocate temporary lease structure: %i", msg, result);//ScenSim-Port//
		free_lease_state (state, MDL);
		if (host)
			host_dereference (&host, MDL);
		return;
...
//ScenSim-Port//		lt -> client_hostname = dmalloc (d1.len + 1, MDL);
		lt -> client_hostname = (char*)dmalloc (d1.len + 1, MDL);//ScenSim-Port//
	...
	if (offer == DHCPOFFER && !(lease -> flags & STATIC_LEASE) &&
	    ...
	    (!(oc = lookup_option (&server_universe, state -> options,
	     ...
	     evaluate_boolean_option_cache (&ignorep, packet, lease,
					    ...
					    state -> options,
					    &lease -> scope, oc, MDL))) {
//ScenSim-Port//		icmp_echorequest (&lease -> ip_addr);
...
//ScenSim-Port//void dhcp_reply (lease)
//ScenSim-Port//	struct lease *lease;
void dhcp_reply (struct lease *lease)//ScenSim-Port//
{
	int bufs = 0;
...
//ScenSim-Port//		lease -> uid = dmalloc (rhp -> client_identifier.len, MDL);
		lease -> uid = (unsigned char*)dmalloc (rhp -> client_identifier.len, MDL);//ScenSim-Port//
	else
...
//ScenSim-Port//int permitted (packet, permit_list)
//ScenSim-Port//	struct packet *packet;
//ScenSim-Port//	struct permit *permit_list;
int permitted (struct packet *packet, struct permit *permit_list)//ScenSim-Port//
{
	struct permit *p;
	int i;
	...
	for (p = permit_list; p; p = p -> next) {
		switch (p -> type) {
			...
			break;
		      ...
		      case permit_class:
			for (i = 0; i < packet -> class_count; i++) {
//ScenSim-Port//				if (p -> class == packet -> classes [i])
				if (p -> class_ == packet -> classes [i])//ScenSim-Port//
					return 1;
				if (packet -> classes [i] &&
				    packet -> classes [i] -> superclass &&
				    (packet -> classes [i] -> superclass ==
//ScenSim-Port//				     p -> class))
				     p -> class_))//ScenSim-Port//
					return 1;
			}
			break;
...
//ScenSim-Port//int locate_network (packet)
//ScenSim-Port//	struct packet *packet;
int locate_network (struct packet *packet)//ScenSim-Port//
{
	struct iaddr ia;
	struct data_string data;
...
find_min_site_code(struct universe *u)
{
	 ...
	 * RFC 3942.
	 */
//ScenSim-Port//	site_code_min = 224;
	site_code_min_ = 224;//ScenSim-Port//
	option_code_hash_foreach(u->code_hash, lowest_site_code);

//ScenSim-Port//	if (site_code_min < 224) {
	if (site_code_min_ < 224) {//ScenSim-Port//
...
//ScenSim-Port//			  u->name, site_code_min);
			  u->name, site_code_min_);//ScenSim-Port//
	}
	 ...
	 */
//ScenSim-Port//	if (site_code_min < 128)
	if (site_code_min_ < 128)//ScenSim-Port//
//ScenSim-Port//		site_code_min = 128;
		site_code_min_ = 128;//ScenSim-Port//
	 ...
	 * impossible.
	 */
//ScenSim-Port//	u->site_code_min = site_code_min;
	u->site_code_min = site_code_min_;//ScenSim-Port//

//ScenSim-Port//	return site_code_min;
	return site_code_min_;//ScenSim-Port//
}

static isc_result_t
lowest_site_code(const void *key, unsigned len, void *object)
{
//ScenSim-Port//	struct option *option = object;
	struct option *option = (struct option*)object;//ScenSim-Port//

//ScenSim-Port//	if (option->code < site_code_min)
	if (option->code < site_code_min_)//ScenSim-Port//
//ScenSim-Port//		site_code_min = option->code;
		site_code_min_ = option->code;//ScenSim-Port//
