...
//ScenSim-Port//static isc_result_t write_binding_scope(FILE *db_file, struct binding *bnd,
//ScenSim-Port//					char *prepend);
static isc_result_t write_binding_scope(FILE *db_file_arg, struct binding *bnd,//ScenSim-Port//
					const char *prepend);//ScenSim-Port//

//ScenSim-Port//FILE *db_file;

//ScenSim-Port//static int counting = 0;
#define counting SCENSIMGLOBAL(counting)//ScenSim-Port//
//ScenSim-Port//static int count = 0;
#define count SCENSIMGLOBAL(count)//ScenSim-Port//
//ScenSim-Port//TIME write_time;
//ScenSim-Port//int lease_file_is_corrupt = 0;

/* Write a single binding scope value in parsable format.
 */

static isc_result_t
//ScenSim-Port//write_binding_scope(FILE *db_file, struct binding *bnd, char *prepend) {
write_binding_scope(FILE *db_file_arg, struct binding *bnd, const char *prepend) {//ScenSim-Port//
	char *s;

//ScenSim-Port//	if ((db_file == NULL) || (bnd == NULL) || (prepend == NULL))
	if ((db_file_arg == NULL) || (bnd == NULL) || (prepend == NULL))//ScenSim-Port//
	...
	if (bnd->value->type == binding_data) {
		if (bnd->value->value.data.data != NULL) {
			...
			if (s != NULL) {
				errno = 0;
//ScenSim-Port//				fprintf(db_file, "%sset %s = \"%s\";",
				fprintf(db_file_arg, "%sset %s = \"%s\";",//ScenSim-Port//
					prepend, bnd->name, s);
				if (errno)
		...
		errno = 0;
//ScenSim-Port//		fprintf(db_file, "%sset %s = %%%ld;", prepend,
		fprintf(db_file_arg, "%sset %s = %%%ld;", prepend,//ScenSim-Port//
			bnd->name, bnd->value->value.intval);
		if (errno)
			return ISC_R_FAILURE;
	} else if (bnd->value->type == binding_boolean) {
		errno = 0;
//ScenSim-Port//		fprintf(db_file, "%sset %s = %s;", prepend, bnd->name,
		fprintf(db_file_arg, "%sset %s = %s;", prepend, bnd->name,//ScenSim-Port//
...
int write_lease (struct lease *lease)//ScenSim-Port//
{
	int errors = 0;
	struct binding *b;
	char *s;
...
int write_host (struct host_decl *host)//ScenSim-Port//
{
	int errors = 0;
	int i;
...
//ScenSim-Port//int write_group (group)
//ScenSim-Port//	struct group_object *group;
int write_group (struct group_object *group)//ScenSim-Port//
{
	int errors = 0;
...
//ScenSim-Port//int db_printable (s)
//ScenSim-Port//	const unsigned char *s;
int db_printable (const unsigned char *s)//ScenSim-Port//
{
	int i;
	for (i = 0; s [i]; i++)
...
//ScenSim-Port//int db_printable_len (s, len)
//ScenSim-Port//	const unsigned char *s;
//ScenSim-Port//	unsigned len;
int db_printable_len (const unsigned char *s, unsigned len)//ScenSim-Port//
{
	int i;
...
//ScenSim-Port//static int print_hash_string(FILE *fp, struct class *class)
static int print_hash_string(FILE *fp, struct class_type *class)//ScenSim-Port//
{
	int i;
...
{
//ScenSim-Port//	const unsigned char *name = key;
	const unsigned char *name = (const unsigned char*)key;//ScenSim-Port//
//ScenSim-Port//	struct class *class = object;
	struct class_type *class = (struct class_type*)object;//ScenSim-Port//
...
//ScenSim-Port//int write_billing_class (class)
//ScenSim-Port//	struct class *class;
int write_billing_class (struct class_type *class)//ScenSim-Port//
{
	int errors = 0;
...
void db_startup (int testp)//ScenSim-Port//
{
	isc_result_t status;
...
//ScenSim-Port//	fprintf (db_file, "# This lease file was written by isc-dhcp-%s\n\n",
//ScenSim-Port//		 PACKAGE_VERSION);
//ScenSim-Port//	if (errno)
//ScenSim-Port//		goto fail;
