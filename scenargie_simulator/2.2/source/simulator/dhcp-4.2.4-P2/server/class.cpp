...
static struct collection default_collection = {//ScenSim-Port//
	(struct collection *)0,
	"default",
//ScenSim-Port//	(struct class *)0,
	(struct class_type *)0,//ScenSim-Port//
};

//ScenSim-Port//struct collection *collections = &default_collection;
//ScenSim-Port//struct executable_statement *default_classification_rules;

//ScenSim-Port//int have_billing_classes;
...
void classification_setup ()
{
	...
	default_classification_rules -> data.eval -> data.check =
		&default_collection;
	collections = &default_collection;//ScenSim-Port//
}

//ScenSim-Port//void classify_client (packet)
//ScenSim-Port//	struct packet *packet;
void classify_client (struct packet *packet)//ScenSim-Port//
{
...
int check_collection (struct packet *packet, struct lease *lease, struct collection *collection)//ScenSim-Port//
{
//ScenSim-Port//	struct class *class, *nc;
	struct class_type *class, *nc;//ScenSim-Port//
		...
		if (class -> submatch) {
			status = (evaluate_data_expression
				  (&data, packet, lease,
				   ...
				   class -> submatch, MDL));
			if (status && data.len) {
//ScenSim-Port//				nc = (struct class *)0;
				nc = (struct class_type *)0;//ScenSim-Port//
				...
				if (nc -> lease_limit) {
					nc -> billed_leases =
//ScenSim-Port//						(dmalloc
						(struct lease**)(dmalloc//ScenSim-Port//
						 (nc -> lease_limit *
						  sizeof (struct lease *),
						  MDL));
					if (!nc -> billed_leases) {
...
void classify (struct packet *packet, struct class_type *class)//ScenSim-Port//
{
...
isc_result_t unlink_class(struct class_type **class) {//ScenSim-Port//
	struct collection *lp;
//ScenSim-Port//	struct class *cp, *pp;
	struct class_type *cp, *pp;//ScenSim-Port//
...
//ScenSim-Port//isc_result_t find_class (struct class **class, const char *name,
isc_result_t find_class (struct class_type **class, const char *name,//ScenSim-Port//
...
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct class *class;
int unbill_class (struct lease *lease, struct class_type *class)//ScenSim-Port//
{
	int i;
...
//ScenSim-Port//int bill_class (lease, class)
//ScenSim-Port//	struct lease *lease;
//ScenSim-Port//	struct class *class;
int bill_class (struct lease *lease, struct class_type *class)//ScenSim-Port//
{
	int i;
