...
#define class class_//ScenSim-Port//
#define new new_//ScenSim-Port//

//ScenSim-Port//#include "dhcpd.h"

//ScenSim-Port//static unsigned char global_host_once = 1;
//ScenSim-Port//static unsigned char dhcpv6_class_once = 1;
...
//ScenSim-Port//int parse_statement (cfile, group, type, host_decl, declaration)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct group *group;
//ScenSim-Port//	int type;
//ScenSim-Port//	struct host_decl *host_decl;
//ScenSim-Port//	int declaration;
int parse_statement (struct parse *cfile, struct group *group, int type, struct host_decl *host_decl, int declaration)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
	...
	switch (token) {
		...
		if (status != ISC_R_SUCCESS)
//ScenSim-Port//			log_fatal ("Can't allocate shared subnet: %s",
//ScenSim-Port//				   isc_result_totext (status));
			log_fatal ("Can't allocate shared subnet: %i", status);//ScenSim-Port//
...
//ScenSim-Port//void parse_pool_statement (cfile, group, type)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct group *group;
//ScenSim-Port//	int type;
void parse_pool_statement (struct parse *cfile, struct group *group, int type)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
	int done = 0;
	...
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("no memory for pool: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("no memory for pool: %i", status);//ScenSim-Port//
...
//ScenSim-Port//				permit -> class = (struct class *)0;
				permit -> class = (struct class_type *)0;//ScenSim-Port//
...
//ScenSim-Port//int parse_lbrace (cfile)
//ScenSim-Port//	struct parse *cfile;
int parse_lbrace (struct parse *cfile)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
...
//ScenSim-Port//void parse_host_declaration (cfile, group)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct group *group;
void parse_host_declaration (struct parse *cfile, struct group *group)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
	...
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("can't allocate host decl struct %s: %s",
//ScenSim-Port//			   name, isc_result_totext (status));
		log_fatal ("can't allocate host decl struct %s: %i", name, status);//ScenSim-Port//
	host -> name = name;
	...
	if (deleted) {
		...
		if (status != ISC_R_SUCCESS)
//ScenSim-Port//			parse_warn (cfile, "host %s: %s", host -> name,
//ScenSim-Port//				    isc_result_totext (status));
			parse_warn (cfile, "host %s: %i", host -> name, status);//ScenSim-Port//
	}
	host_dereference (&host, MDL);
}
...
//ScenSim-Port//int parse_class_declaration (cp, cfile, group, type)
//ScenSim-Port//	struct class **cp;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct group *group;
//ScenSim-Port//	int type;
int parse_class_declaration (struct class_type **cp, struct parse *cfile, struct group *group, int type)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
//ScenSim-Port//	struct class *class = (struct class *)0, *pc = (struct class *)0;
	struct class_type *class = (struct class_type *)0, *pc = (struct class_type *)0;//ScenSim-Port//
	int declaration = 0;
	int lose = 0;
	...
	if (tname) {
//ScenSim-Port//		name = dmalloc (strlen (tname) + 1, MDL);
		name = (char*)dmalloc (strlen (tname) + 1, MDL);//ScenSim-Port//
		if (!name)
	...
	if (!class) {
		...
		if (pc) {
			...
			if (class -> lease_limit) {
				class -> billed_leases =
//ScenSim-Port//					dmalloc (class -> lease_limit *
					(lease**)dmalloc (class -> lease_limit *//ScenSim-Port//
...
//ScenSim-Port//					(void *)class, MDL);
					class, MDL);//ScenSim-Port//
		} else {
			if (class->group)
...
//ScenSim-Port//				dmalloc (class -> lease_limit *
	...
	if (class->flags & CLASS_DECL_DELETED) {
		if (type == CLASS_TYPE_CLASS) {
//ScenSim-Port//			struct class *theclass = NULL;
			struct class_type *theclass = NULL;//ScenSim-Port//
...
//ScenSim-Port//			struct class *c;
			struct class_type *c;//ScenSim-Port//
...
//ScenSim-Port//void parse_shared_net_declaration (cfile, group)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct group *group;
void parse_shared_net_declaration (struct parse *cfile, struct group *group)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
	struct shared_network *share;
...
//ScenSim-Port//void parse_subnet_declaration (cfile, share)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct shared_network *share;
void parse_subnet_declaration (struct parse *cfile, struct shared_network *share)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
	struct subnet *subnet;
	...
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Allocation of new subnet failed: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Allocation of new subnet failed: %i", status);//ScenSim-Port//
...
//ScenSim-Port//void parse_group_declaration (cfile, group)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct group *group;
void parse_group_declaration (struct parse *cfile, struct group *group)//ScenSim-Port//
{
	const char *val;
	enum dhcp_token token;
	struct group *g;
...
//ScenSim-Port//		name = dmalloc(strlen(val) + 1, MDL);
	...
	if (name) {
		if (deletedp) {
			...
			if (status != ISC_R_SUCCESS)
//ScenSim-Port//				log_fatal("no memory for group decl %s: %s",
//ScenSim-Port//					  val, isc_result_totext(status));
				log_fatal("no memory for group decl %s: %i", val, status);//ScenSim-Port//
...
//ScenSim-Port//			    binding = dmalloc (sizeof *binding, MDL);
			    ...
			    binding -> name =
//ScenSim-Port//				    dmalloc (strlen (val) + 1, MDL);
				    (char*)dmalloc (strlen (val) + 1, MDL);//ScenSim-Port//
			    if (!binding -> name)
...
//ScenSim-Port//void parse_address_range (cfile, group, type, inpool, lpchain)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct group *group;
//ScenSim-Port//	int type;
//ScenSim-Port//	struct pool *inpool;
//ScenSim-Port//	struct lease **lpchain;
void parse_address_range (struct parse *cfile, struct group *group, int type, struct pool *inpool, struct lease **lpchain)//ScenSim-Port//
{
	...
	if (!inpool) {
		...
		if (!pool) {
			...
			status = pool_allocate (&pool, MDL);
			if (status != ISC_R_SUCCESS)
//ScenSim-Port//				log_fatal ("no memory for ad-hoc pool: %s",
//ScenSim-Port//					   isc_result_totext (status));
				log_fatal ("no memory for ad-hoc pool: %i", status);//ScenSim-Port//
			p = new_permit (MDL);
			if (!p)
...
#pragma clang diagnostic push //ScenSim-Port//
#pragma clang diagnostic ignored "-Wunused-value" //ScenSim-Port//
	int num_pools;
#pragma clang diagnostic pop //ScenSim-Port//
...
//ScenSim-Port//	tmp = dmalloc(sizeof(struct ipv6_pool *) * (num_pools + 2), MDL);
	tmp = (ipv6_pool**)dmalloc(sizeof(struct ipv6_pool *) * (num_pools + 2), MDL);//ScenSim-Port//
	if (tmp == NULL) {
		log_fatal("Out of memory");
	}
	if (num_pools > 0) {
...
//ScenSim-Port//	ia = dmalloc(sizeof(*ia), MDL);
	ia = (iaddrcidrnetlist*)dmalloc(sizeof(*ia), MDL);//ScenSim-Port//
	if (!ia) {
		log_fatal("Out of memory");
	}
...
int parse_allow_deny (struct option_cache **oc, struct parse *cfile, int flag)//ScenSim-Port//
{
	enum dhcp_token token;
	const char *val;
	...
	for (;;) {
		...
		for (;;) {
			...
			switch(token) {
				...
				if (bnd == NULL) {
//ScenSim-Port//					bnd = dmalloc(sizeof(*bnd),
					bnd = (binding*)dmalloc(sizeof(*bnd),//ScenSim-Port//
							  MDL);
					if (bnd == NULL) {
...
//ScenSim-Port//					bnd->name = dmalloc(strlen(val) + 1,
					bnd->name = (char*)dmalloc(strlen(val) + 1,//ScenSim-Port//
							    MDL);
					if (bnd->name == NULL) {
						log_fatal("No memory for "
							  "binding name.");
