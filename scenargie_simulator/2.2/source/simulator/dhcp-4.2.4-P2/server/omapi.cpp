...
//ScenSim-Port//omapi_object_type_t *dhcp_type_lease;
//ScenSim-Port//omapi_object_type_t *dhcp_type_pool;
//ScenSim-Port//omapi_object_type_t *dhcp_type_class;
//ScenSim-Port//omapi_object_type_t *dhcp_type_subclass;
...
//ScenSim-Port//		log_fatal ("Can't register lease object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register lease object type: %i", status);
...
//ScenSim-Port//					     sizeof (struct class), 0,
					     sizeof (struct class_type), 0,//ScenSim-Port//
					     RC_MISC);
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register class object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register class object type: %i", status);//ScenSim-Port//
...
//ScenSim-Port//					     sizeof (struct class), 0, RC_MISC);
					     sizeof (struct class_type), 0, RC_MISC);//ScenSim-Port//
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register subclass object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register subclass object type: %i", status);//ScenSim-Port//
	...
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register pool object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register pool object type: %i", status);//ScenSim-Port//
	...
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register host object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register host object type: %i", status);//ScenSim-Port//
	...
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register failover state object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register failover state object type: %i", status);//ScenSim-Port//
	...
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register failover link object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register failover link object type: %i", status);//ScenSim-Port//
	...
	if (status != ISC_R_SUCCESS)
//ScenSim-Port//		log_fatal ("Can't register failover listener object type: %s",
//ScenSim-Port//			   isc_result_totext (status));
		log_fatal ("Can't register failover listener object type: %i", status);//ScenSim-Port//
...
//ScenSim-Port//		haddr = dmalloc (len, MDL);
		haddr = (unsigned char*)dmalloc (len, MDL);//ScenSim-Port//
		if (!haddr) {
			omapi_value_dereference (&tv, MDL);
			return ISC_R_NOMEMORY;
		}
...
//ScenSim-Port//			host -> name = dmalloc (value -> u.buffer.len + 1,
			host -> name = (char*)dmalloc (value -> u.buffer.len + 1,//ScenSim-Port//
						MDL);
			if (!host -> name)
				return ISC_R_NOMEMORY;
			memcpy (host -> name,
...
//ScenSim-Port//			host -> name = dmalloc (strlen (hnbuf) + 1, MDL);
			host -> name = (char*)dmalloc (strlen (hnbuf) + 1, MDL);//ScenSim-Port//
			if (!host -> name)
				return ISC_R_NOMEMORY;
...
//ScenSim-Port//	struct class *superclass = 0;
	struct class_type *superclass = 0;//ScenSim-Port//
	isc_result_t status;
	int issubclass = (h -> type == dhcp_type_subclass);

//ScenSim-Port//	class = (struct class *)h;
	class = (struct class_type *)h;//ScenSim-Port//
...
//ScenSim-Port//		if ((tname = dmalloc(value->u.buffer.len + 1, MDL)) == NULL) {
		if ((tname = (char*)dmalloc(value->u.buffer.len + 1, MDL)) == NULL) {//ScenSim-Port//
			return ISC_R_NOMEMORY;
		}
		...
		if (issubclass) {
			...
			class_reference(&class->superclass, superclass, MDL);
			   ...
			   value -> type == omapi_datatype_string) {
//ScenSim-Port//			class->name = dmalloc(value->u.buffer.len + 1, MDL);
			class->name = (char*)dmalloc(value->u.buffer.len + 1, MDL);//ScenSim-Port//
			if (!class->name)
				return ISC_R_NOMEMORY;
...
#if defined (DEBUG_MEMORY_LEAKAGE) || \
		defined (DEBUG_MEMORY_LEAKAGE_ON_EXIT)
//ScenSim-Port//	struct class *class = (struct class *)h;
	struct class_type *class = (struct class_type *)h;//ScenSim-Port//
	if (class -> nic)
...
//ScenSim-Port//				  (void *)class, MDL);
				  (hashed_object_t*)class, MDL);//ScenSim-Port//
		}
				  ...
				  omapi_object_type_t *typewanted)
{
	...
	isc_result_t status;
//ScenSim-Port//	struct class *class = 0;
	struct class_type *class = 0;//ScenSim-Port//
//ScenSim-Port//	struct class *subclass = 0;
	struct class_type *subclass = 0;//ScenSim-Port//
	...
	*lp = NULL;
	...
	if (status == ISC_R_SUCCESS) {
//ScenSim-Port//		char *name = dmalloc(nv -> value -> u.buffer.len + 1, MDL);
		char *name = (char*)dmalloc(nv -> value -> u.buffer.len + 1, MDL);//ScenSim-Port//
		memcpy (name,
...
isc_result_t dhcp_class_create (omapi_object_t **lp,
				omapi_object_t *id)
{
//ScenSim-Port//	struct class *cp = 0;
	struct class_type *cp = 0;//ScenSim-Port//
	isc_result_t status;
...
//ScenSim-Port//	struct class *cp;
	...
	if (lp -> type != dhcp_type_class)
		return DHCP_R_INVALIDARG;
//ScenSim-Port//	cp = (struct class *)lp;
	cp = (struct class_type *)lp;//ScenSim-Port//
...
isc_result_t dhcp_subclass_get_value (omapi_object_t *h, omapi_object_t *id,
				      ...
				      omapi_value_t **value)
{
//ScenSim-Port//	struct class *subclass;
	struct class_type *subclass;//ScenSim-Port//
	isc_result_t status;

	if (h -> type != dhcp_type_class)
		return DHCP_R_INVALIDARG;
//ScenSim-Port//	subclass = (struct class *)h;
	subclass = (struct class_type *)h;//ScenSim-Port//
	if (subclass -> name != 0)
		return DHCP_R_INVALIDARG;
...
//ScenSim-Port//	nname = dmalloc (name -> len + 1, MDL);
	nname = (char*)dmalloc (name -> len + 1, MDL);//ScenSim-Port//
	if (!nname)
		return ISC_R_NOMEMORY;
	...
	if (!bp) {
//ScenSim-Port//		bp = dmalloc (sizeof *bp, MDL);
		bp = (binding*)dmalloc (sizeof *bp, MDL);//ScenSim-Port//
		if (!bp) {
			binding_value_dereference (&nv, MDL);
			dfree (nname, MDL);
			return ISC_R_NOMEMORY;
