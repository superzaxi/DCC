...
//ScenSim-Port//#include <stdarg.h>
//ScenSim-Port//#include "dhcpd.h"
//ScenSim-Port//#include "omapip/omapip.h"
//ScenSim-Port//#include "omapip/hash.h"
//ScenSim-Port//#include <isc/md5.h>

HASH_FUNCTIONS(ia, unsigned char *, struct ia_xx, ia_hash_t,
	       ia_reference, ia_dereference, do_string_hash)

//ScenSim-Port//ia_hash_t *ia_na_active;
//ScenSim-Port//ia_hash_t *ia_ta_active;
//ScenSim-Port//ia_hash_t *ia_pd_active;
...
//ScenSim-Port//struct ipv6_pool **pools;
#define pools SCENSIMGLOBAL(pools)//ScenSim-Port//
//ScenSim-Port//int num_pools;
...
//ScenSim-Port//	tmp = dmalloc(sizeof(*tmp), file, line);
	tmp = (struct iasubopt*)dmalloc(sizeof(*tmp), file, line);//ScenSim-Port//
	if (tmp == NULL) {
...
//ScenSim-Port//	tmp = dmalloc(sizeof(*tmp), file, line);
	tmp = (ia_xx*)dmalloc(sizeof(*tmp), file, line);//ScenSim-Port//
	if (tmp == NULL) {
		return ISC_R_NOMEMORY;
	}
 ...
 */
#define new new_local//ScenSim-Port//
isc_result_t
...
//ScenSim-Port//		new = dmalloc(max * sizeof(struct iasubopt *), file, line);
		new = (struct iasubopt**)dmalloc(max * sizeof(struct iasubopt *), file, line);//ScenSim-Port//
		if (new == NULL) {
			return ISC_R_NOMEMORY;
		}
...
//ScenSim-Port//		return difftime(la->soft_lifetime_end_time,
//ScenSim-Port//				lb->soft_lifetime_end_time) < 0;
		return (isc_boolean_t)(difftime(la->soft_lifetime_end_time,//ScenSim-Port//
				lb->soft_lifetime_end_time) < 0);//ScenSim-Port//
	} else {
//ScenSim-Port//		return difftime(la->hard_lifetime_end_time, 
//ScenSim-Port//				lb->hard_lifetime_end_time) < 0;
		return (isc_boolean_t)(difftime(la->hard_lifetime_end_time, //ScenSim-Port//
				lb->hard_lifetime_end_time) < 0);//ScenSim-Port//
	}
}
...
//ScenSim-Port//	tmp = dmalloc(sizeof(*tmp), file, line);
	tmp = (ipv6_pool*)dmalloc(sizeof(*tmp), file, line);//ScenSim-Port//
	if (tmp == NULL) {
		return ISC_R_NOMEMORY;
	}
...
//ScenSim-Port//	if (isc_heap_create(dhcp_gbl_ctx.mctx, lease_older, lease_index_changed,
	if (isc_heap_create(lease_older, lease_index_changed,//ScenSim-Port//
...
build_temporary6(struct in6_addr *addr, 
	 ...
	 */
	if (counter == 0) {
//ScenSim-Port//		isc_random_get(&history[0]);
//ScenSim-Port//		isc_random_get(&history[1]);
        history[0] = GenerateRandomInt(curctx->glue);//ScenSim-Port//
        history[0] = GenerateRandomInt(curctx->glue);//ScenSim-Port//
	}
...
//ScenSim-Port//				 (void *)addr, sizeof(*addr), MDL)) {
				 (in6_addr*)addr, sizeof(*addr), MDL)) {//ScenSim-Port//
		iasubopt_dereference(&test_iaaddr, MDL);
		return ISC_TRUE;
	} else {
		return ISC_FALSE;
...
//ScenSim-Port//				 (void *)&lease->addr,
				 (in6_addr*)&lease->addr,//ScenSim-Port//
				 sizeof(lease->addr), MDL)) {
...
prefix6_exists(const struct ipv6_pool *pool,
	...
	test_iapref = NULL;
	if (iasubopt_hash_lookup(&test_iapref, pool->leases, 
//ScenSim-Port//				 (void *)pref, sizeof(*pref), MDL)) {
				 (in6_addr*)pref, sizeof(*pref), MDL)) {//ScenSim-Port//
		iasubopt_dereference(&test_iapref, MDL);
		return ISC_TRUE;
	} else {
		return ISC_FALSE;
...
//ScenSim-Port//	new_pools = dmalloc(sizeof(struct ipv6_pool *) * (num_pools+1), MDL);
	new_pools = (ipv6_pool**)dmalloc(sizeof(struct ipv6_pool *) * (num_pools+1), MDL);//ScenSim-Port//
	if (new_pools == NULL) {
		return ISC_R_NOMEMORY;
	}
