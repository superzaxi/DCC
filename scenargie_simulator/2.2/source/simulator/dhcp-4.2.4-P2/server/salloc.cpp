...
//ScenSim-Port//struct lease *new_leases (n, file, line)
//ScenSim-Port//	unsigned n;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
struct lease *new_leases (unsigned n, const char *file, int line)//ScenSim-Port//
{
	struct lease *rval;
#if defined (DEBUG_MEMORY_LEAKAGE_ON_EXIT)
...
//ScenSim-Port//OMAPI_OBJECT_ALLOC (class, struct class, dhcp_type_class)
OMAPI_OBJECT_ALLOC (class, struct class_type, dhcp_type_class)//ScenSim-Port//
//ScenSim-Port//OMAPI_OBJECT_ALLOC (subclass, struct class, dhcp_type_subclass)
OMAPI_OBJECT_ALLOC (subclass, struct class_type, dhcp_type_subclass)//ScenSim-Port//
...
//ScenSim-Port//struct lease_state *new_lease_state (file, line)
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
struct lease_state *new_lease_state (const char *file, int line)//ScenSim-Port//
{
...
//ScenSim-Port//		rval = dmalloc (sizeof (struct lease_state), file, line);
		rval = (struct lease_state*)dmalloc (sizeof (struct lease_state), file, line);//ScenSim-Port//
		if (!rval)
			return rval;
	}
...
//ScenSim-Port//void free_lease_state (ptr, file, line)
//ScenSim-Port//	struct lease_state *ptr;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
void free_lease_state (struct lease_state *ptr, const char *file, int line)//ScenSim-Port//
{
	if (ptr -> options)
...
//ScenSim-Port//struct permit *new_permit (file, line)
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
struct permit *new_permit (const char *file, int line)//ScenSim-Port//
{
...
//ScenSim-Port//void free_permit (permit, file, line)
//ScenSim-Port//	struct permit *permit;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
void free_permit (struct permit *permit, const char *file, int line)//ScenSim-Port//
{
	if (permit -> type == permit_class)
//ScenSim-Port//		class_dereference (&permit -> class, MDL);
		class_dereference (&permit -> class_, MDL);//ScenSim-Port//
