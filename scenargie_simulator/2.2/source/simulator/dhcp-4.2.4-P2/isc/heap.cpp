...
#define REQUIRE assert//ScenSim-Port//
#define INSIST assert//ScenSim-Port//
#define RUNTIME_CHECK assert//ScenSim-Port//

//ScenSim-Port//#include <config.h>

//ScenSim-Port//#include <isc/heap.h>
//ScenSim-Port//#include <isc/magic.h>
//ScenSim-Port//#include <isc/mem.h>
//ScenSim-Port//#include <isc/string.h>		/* Required for memcpy. */
//ScenSim-Port//#include <isc/util.h>
...
//ScenSim-Port//#define HEAP_MAGIC			ISC_MAGIC('H', 'E', 'A', 'P')
//ScenSim-Port//#define VALID_HEAP(h)			ISC_MAGIC_VALID(h, HEAP_MAGIC)
...
/*% ISC heap structure. */
struct isc_heap {
//ScenSim-Port//	unsigned int			magic;
...
isc_result_t
//ScenSim-Port//isc_heap_create(isc_mem_t *mctx, isc_heapcompare_t compare,
isc_heap_create(isc_heapcompare_t compare,//ScenSim-Port//
...
//ScenSim-Port//	heap = isc_mem_get(mctx, sizeof(*heap));
    heap = (isc_heap_t*)new char[sizeof(*heap)];//ScenSim-Port//
	if (heap == NULL)
		return (ISC_R_NOMEMORY);
//ScenSim-Port//	heap->magic = HEAP_MAGIC;
	heap->size = 0;
//ScenSim-Port//	heap->mctx = NULL;
//ScenSim-Port//	isc_mem_attach(mctx, &heap->mctx);
	if (size_increment == 0)
...
isc_heap_destroy(isc_heap_t **heapp) {
	...
	REQUIRE(heapp != NULL);
	heap = *heapp;
//ScenSim-Port//	REQUIRE(VALID_HEAP(heap));

	if (heap->array != NULL)
//ScenSim-Port//		isc_mem_put(heap->mctx, heap->array,
//ScenSim-Port//			    heap->size * sizeof(void *));
        delete [] heap->array;//ScenSim-Port//
//ScenSim-Port//	heap->magic = 0;
//ScenSim-Port//	isc_mem_putanddetach(&heap->mctx, heap, sizeof(*heap));
    delete [] heap;//ScenSim-Port//
...
//ScenSim-Port//	new_array = isc_mem_get(heap->mctx, new_size * sizeof(void *));
    new_array = (void**)new char[new_size * sizeof(void*)];//ScenSim-Port//
	if (new_array == NULL)
		return (ISC_FALSE);
	if (heap->array != NULL) {
