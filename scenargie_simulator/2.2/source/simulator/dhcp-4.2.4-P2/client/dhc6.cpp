...
//ScenSim-Port//struct sockaddr_in6 DHCPv6DestAddr;
static struct sockaddr_in6 DHCPv6DestAddr;//ScenSim-Port//

/*
 * Option definition structures that are used by the software - declared
 * here once and assigned at startup to save lookups.
 */
//ScenSim-Port//struct option *clientid_option = NULL;
//ScenSim-Port//struct option *elapsed_option = NULL;
//ScenSim-Port//struct option *ia_na_option = NULL;
//ScenSim-Port//struct option *ia_ta_option = NULL;
//ScenSim-Port//struct option *ia_pd_option = NULL;
//ScenSim-Port//struct option *iaaddr_option = NULL;
//ScenSim-Port//struct option *iaprefix_option = NULL;
//ScenSim-Port//struct option *oro_option = NULL;
//ScenSim-Port//struct option *irt_option = NULL;
...
//ScenSim-Port//static void script_write_params6(struct client_state *client,
...
//ScenSim-Port//extern int onetry;
//ScenSim-Port//extern int stateless;
...
//ScenSim-Port//		path_dhclient_pid = _PATH_DHCLIENT6_PID;
//ScenSim-Port//	if (path_dhclient_db == NULL)
//ScenSim-Port//		path_dhclient_db = _PATH_DHCLIENT6_DB;
...
//ScenSim-Port//	copy = dmalloc(sizeof(*copy), file, line);
	copy = (dhc6_lease*)dmalloc(sizeof(*copy), file, line);//ScenSim-Port//
	if (copy == NULL) {
...
//ScenSim-Port//	copy = dmalloc(sizeof(*ia), file, line);
	copy = (dhc6_ia*)dmalloc(sizeof(*ia), file, line);//ScenSim-Port//
...
//ScenSim-Port//	copy = dmalloc(sizeof(*addr), file, line);
	copy = (dhc6_addr*)dmalloc(sizeof(*addr), file, line);//ScenSim-Port//
...
//ScenSim-Port//	lease = dmalloc(sizeof(*lease), MDL);
	lease = (dhc6_lease*)dmalloc(sizeof(*lease), MDL);//ScenSim-Port//
	if (lease == NULL) {
		log_error("Out of memory for v6 lease structure.");
		return NULL;
	}
...
//ScenSim-Port//		ia = dmalloc(sizeof(*ia), MDL);
		ia = (dhc6_ia*)dmalloc(sizeof(*ia), MDL);//ScenSim-Port//
		if (ia == NULL) {
		 ...
		 struct option_state *options)
{
	...
	for ( ; oc != NULL ; oc = oc->next) {
//ScenSim-Port//		addr = dmalloc(sizeof(*addr), MDL);
		addr = (dhc6_addr*)dmalloc(sizeof(*addr), MDL);//ScenSim-Port//
		if (addr == NULL) {
			log_error("Out of memory allocating "
				  "address structure.");
			return ISC_R_NOMEMORY;
		    ...
		    struct option_state *options)
{
	...
	for ( ; oc != NULL ; oc = oc->next) {
//ScenSim-Port//		pfx = dmalloc(sizeof(*pfx), MDL);
		pfx = (dhc6_addr*)dmalloc(sizeof(*pfx), MDL);//ScenSim-Port//
		if (pfx == NULL) {
			log_error("Out of memory allocating "
				  "prefix structure.");
			return ISC_R_NOMEMORY;
...
//ScenSim-Port//	if (!onetry)
		client->MRD = 0;
//ScenSim-Port//	else
//ScenSim-Port//		client->MRD = client->config->timeout;
...
//ScenSim-Port//	client = input;
	client = (client_state*)input;//ScenSim-Port//
...
//ScenSim-Port//	switch(check_timing6(client, DHCPV6_SOLICIT, "Solicit", NULL, &ds)) {
	switch(check_timing6(client, DHCPV6_SOLICIT, (char*)"Solicit", NULL, &ds)) {//ScenSim-Port//
...
//ScenSim-Port//			     "Info-Request", NULL, &ds)) {
			     (char*)"Info-Request", NULL, &ds)) {//ScenSim-Port//
	      case CHK_TIM_MRC_EXCEEDED:
	      case CHK_TIM_ALLOC_FAILURE:
		return;
	      case CHK_TIM_MRD_EXCEEDED:
...
//ScenSim-Port//	switch(check_timing6(client, DHCPV6_CONFIRM, "Confirm",
	switch(check_timing6(client, DHCPV6_CONFIRM, (char*)"Confirm",//ScenSim-Port//
...
//ScenSim-Port//	switch(check_timing6(client, DHCPV6_RELEASE, "Release", 
	switch(check_timing6(client, DHCPV6_RELEASE, (char*)"Release", //ScenSim-Port//
			     client->active_lease, &ds)) {
	      case CHK_TIM_MRC_EXCEEDED:
	      case CHK_TIM_ALLOC_FAILURE:
	      case CHK_TIM_MRD_EXCEEDED:
...
//ScenSim-Port//	client->active_lease = dmalloc(sizeof(struct dhc6_lease), MDL);
	client->active_lease = (dhc6_lease*)dmalloc(sizeof(struct dhc6_lease), MDL);//ScenSim-Port//
	if (client->active_lease == NULL)
...
//ScenSim-Port//	switch(check_timing6(client, DHCPV6_REQUEST, "Request", lease, &ds)) {
	switch(check_timing6(client, DHCPV6_REQUEST, (char*)"Request", lease, &ds)) {//ScenSim-Port//
	      case CHK_TIM_MRC_EXCEEDED:
	      case CHK_TIM_MRD_EXCEEDED:
...
//ScenSim-Port//dhc6_marshall_values(const char *prefix, struct client_state *client,
//ScenSim-Port//		     struct dhc6_lease *lease, struct dhc6_ia *ia,
//ScenSim-Port//		     struct dhc6_addr *addr)
//ScenSim-Port//{
//ScenSim-Port//	/* Option cache contents, in descending order of
//ScenSim-Port//	 * scope.
//ScenSim-Port//	 */
//ScenSim-Port//	if ((lease != NULL) && (lease->options != NULL))
//ScenSim-Port//		script_write_params6(client, prefix, lease->options);
//ScenSim-Port//	if ((ia != NULL) && (ia->options != NULL))
//ScenSim-Port//		script_write_params6(client, prefix, ia->options);
//ScenSim-Port//	if ((addr != NULL) && (addr->options != NULL))
//ScenSim-Port//		script_write_params6(client, prefix, addr->options);
//ScenSim-Port//
//ScenSim-Port//	/* addr fields. */
//ScenSim-Port//	if (addr != NULL) {
//ScenSim-Port//		if ((ia != NULL) && (ia->ia_type == D6O_IA_PD)) {
//ScenSim-Port//			client_envadd(client, prefix,
//ScenSim-Port//				      "ip6_prefix", "%s/%u",
//ScenSim-Port//				      piaddr(addr->address),
//ScenSim-Port//				      (unsigned) addr->plen);
//ScenSim-Port//		} else {
//ScenSim-Port//			/* Current practice is that all subnets are /64's, but
//ScenSim-Port//			 * some suspect this may not be permanent.
//ScenSim-Port//			 */
//ScenSim-Port//			client_envadd(client, prefix, "ip6_prefixlen",
//ScenSim-Port//				      "%d", 64);
//ScenSim-Port//			client_envadd(client, prefix, "ip6_address",
//ScenSim-Port//				      "%s", piaddr(addr->address));
//ScenSim-Port//		}
//ScenSim-Port//		if ((ia != NULL) && (ia->ia_type == D6O_IA_TA)) {
//ScenSim-Port//			client_envadd(client, prefix,
//ScenSim-Port//				      "ip6_type", "temporary");
//ScenSim-Port//		}
//ScenSim-Port//		client_envadd(client, prefix, "life_starts", "%d",
//ScenSim-Port//			      (int)(addr->starts));
//ScenSim-Port//		client_envadd(client, prefix, "preferred_life", "%d",
//ScenSim-Port//			      (int)(addr->preferred_life));
//ScenSim-Port//		client_envadd(client, prefix, "max_life", "%d",
//ScenSim-Port//			      (int)(addr->max_life));
//ScenSim-Port//	}
//ScenSim-Port//
//ScenSim-Port//	/* ia fields. */
//ScenSim-Port//	if (ia != NULL) {
//ScenSim-Port//		client_envadd(client, prefix, "iaid", "%s",
//ScenSim-Port//			      print_hex_1(4, ia->iaid, 12));
//ScenSim-Port//		client_envadd(client, prefix, "starts", "%d",
//ScenSim-Port//			      (int)(ia->starts));
//ScenSim-Port//		client_envadd(client, prefix, "renew", "%u", ia->renew);
//ScenSim-Port//		client_envadd(client, prefix, "rebind", "%u", ia->rebind);
//ScenSim-Port//	}
//ScenSim-Port//}
...
//ScenSim-Port//			script_init(client, reason, NULL);

//ScenSim-Port//			if (old != NULL)
//ScenSim-Port//				dhc6_marshall_values("old_", client, old,
//ScenSim-Port//						     oldia, oldaddr);
//ScenSim-Port//			dhc6_marshall_values("new_", client, lease, ia, addr);

//ScenSim-Port//			script_go(client);
            SetIpAddress(curctx->glue, 0, &addr->address, NULL);//ScenSim-Port//
            memcpy(client->interface->v6addresses[0].s6_addr, addr->address.iabuf, addr->address.len);//ScenSim-Port//
		}

		/* XXX: maybe we should loop on the old values instead? */
//ScenSim-Port//		if (ia->addrs == NULL) {
//ScenSim-Port//			script_init(client, reason, NULL);
//ScenSim-Port//
//ScenSim-Port//			if (old != NULL)
//ScenSim-Port//				dhc6_marshall_values("old_", client, old,
//ScenSim-Port//						     oldia,
//ScenSim-Port//						     oldia != NULL ?
//ScenSim-Port//							 oldia->addrs : NULL);
//ScenSim-Port//
//ScenSim-Port//			dhc6_marshall_values("new_", client, lease, ia,
//ScenSim-Port//					     NULL);
//ScenSim-Port//
//ScenSim-Port//			script_go(client);
//ScenSim-Port//		}
	...
	/* XXX: maybe we should loop on the old values instead? */
//ScenSim-Port//	if (lease->bindings == NULL) {
//ScenSim-Port//		script_init(client, reason, NULL);
//ScenSim-Port//
//ScenSim-Port//		if (old != NULL)
//ScenSim-Port//			dhc6_marshall_values("old_", client, old,
//ScenSim-Port//					     old->bindings,
//ScenSim-Port//					     (old->bindings != NULL) ?
//ScenSim-Port//						old->bindings->addrs : NULL);
//ScenSim-Port//
//ScenSim-Port//		dhc6_marshall_values("new_", client, lease, NULL, NULL);
//ScenSim-Port//
//ScenSim-Port//		script_go(client);
//ScenSim-Port//	}
...
//ScenSim-Port//				script_init(client, "DEPREF6", NULL);
//ScenSim-Port//				dhc6_marshall_values("cur_", client, lease,
//ScenSim-Port//						     ia, addr);
//ScenSim-Port//				script_go(client);
...
//ScenSim-Port//				script_init(client, "EXPIRE6", NULL);
//ScenSim-Port//				dhc6_marshall_values("old_", client, lease,
//ScenSim-Port//						     ia, addr);
//ScenSim-Port//				script_go(client);
...
unconfigure6(struct client_state *client, const char *reason)
{
	...
	if (stateless) {
//ScenSim-Port//		script_init(client, reason, NULL);
//ScenSim-Port//		if (client->active_lease != NULL)
//ScenSim-Port//			script_write_params6(client, "old_",
//ScenSim-Port//					     client->active_lease->options);
//ScenSim-Port//		script_go(client);
		return;
	}
	...
	for (ia = client->active_lease->bindings ; ia != NULL ; ia = ia->next) {
		...
		for (addr = ia->addrs ; addr != NULL ; addr = addr->next) {
//ScenSim-Port//			script_init(client, reason, NULL);
//ScenSim-Port//			dhc6_marshall_values("old_", client,
//ScenSim-Port//					     client->active_lease, ia, addr);
//ScenSim-Port//			script_go(client);
...
//ScenSim-Port//	script_init(client, "RENEW6", NULL);
//ScenSim-Port//	if (client->old_lease != NULL)
//ScenSim-Port//		script_write_params6(client, "old_",
//ScenSim-Port//				     client->old_lease->options);
//ScenSim-Port//	script_write_params6(client, "new_", client->active_lease->options);
//ScenSim-Port//	script_go(client);
 ...
 */
//ScenSim-Port//static void
//ScenSim-Port//script_write_params6(struct client_state *client, const char *prefix,
//ScenSim-Port//		     struct option_state *options)
//ScenSim-Port//{
//ScenSim-Port//	struct envadd_state es;
//ScenSim-Port//	int i;
//ScenSim-Port//
//ScenSim-Port//	if (options == NULL)
//ScenSim-Port//		return;
//ScenSim-Port//
//ScenSim-Port//	es.client = client;
//ScenSim-Port//	es.prefix = prefix;
//ScenSim-Port//
//ScenSim-Port//	for (i = 0 ; i < options->universe_count ; i++) {
//ScenSim-Port//		option_space_foreach(NULL, NULL, client, NULL, options,
//ScenSim-Port//				     &global_scope, universes[i], &es,
//ScenSim-Port//				     client_option_envadd);
//ScenSim-Port//	}
//ScenSim-Port//}
