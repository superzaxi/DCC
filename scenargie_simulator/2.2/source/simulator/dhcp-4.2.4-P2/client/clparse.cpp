...
//ScenSim-Port//struct client_config top_level_config;

//ScenSim-Port//#define NUM_DEFAULT_REQUESTED_OPTS	9
//ScenSim-Port//struct option *default_requested_options[NUM_DEFAULT_REQUESTED_OPTS + 1];
...
//ScenSim-Port//void parse_client_statement (cfile, ip, config)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct interface_info *ip;
//ScenSim-Port//	struct client_config *config;
void parse_client_statement (struct parse *cfile, struct interface_info *ip, struct client_config *config)//ScenSim-Port//
{
	int token;
	const char *val;
	...
	switch (peek_token (&val, (unsigned *)0, cfile)) {
		...
		return;
	      ...
	      case KEY:
//ScenSim-Port//		next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		if (ip) {
//ScenSim-Port//			/* This may seem arbitrary, but there's a reason for
//ScenSim-Port//			   doing it: the authentication key database is not
//ScenSim-Port//			   scoped.  If we allow the user to declare a key other
//ScenSim-Port//			   than in the outer scope, the user is very likely to
//ScenSim-Port//			   believe that the key will only be used in that
//ScenSim-Port//			   scope.  If the user only wants the key to be used on
//ScenSim-Port//			   one interface, because it's known that the other
//ScenSim-Port//			   interface may be connected to an insecure net and
//ScenSim-Port//			   the secret key is considered sensitive, we don't
//ScenSim-Port//			   want to lull them into believing they've gotten
//ScenSim-Port//			   their way.   This is a bit contrived, but people
//ScenSim-Port//			   tend not to be entirely rational about security. */
//ScenSim-Port//			parse_warn (cfile, "key definition not allowed here.");
//ScenSim-Port//			skip_to_semi (cfile);
//ScenSim-Port//			break;
//ScenSim-Port//		}
//ScenSim-Port//		parse_key (cfile);
...
//ScenSim-Port//		cat_list = dmalloc(sizeof(struct option *) * (i + tmp + 1),
		cat_list = (struct option**)dmalloc(sizeof(struct option *) * (i + tmp + 1),//ScenSim-Port//
				   MDL);
...
//ScenSim-Port//		config -> vendor_space_name = dmalloc (strlen (val) + 1, MDL);
		config -> vendor_space_name = (char*)dmalloc (strlen (val) + 1, MDL);//ScenSim-Port//
...
//ScenSim-Port//			if (!strcmp (universes [i] -> name,
			if (!strcmp (universes_ [i] -> name,//ScenSim-Port//
				     config -> vendor_space_name))
				break;
//ScenSim-Port//		if (i == universe_count) {
		if (i == universe_count_) {//ScenSim-Port//
		...
		token = next_token (&val, (unsigned *)0, cfile);
		token = next_token (&val, (unsigned *)0, cfile);
//ScenSim-Port//		name = dmalloc (strlen (val) + 1, MDL);
		name = (char*)dmalloc (strlen (val) + 1, MDL);//ScenSim-Port//
		if (!name)
		...
		if (!parse_executable_statement (&stmt,
			...
			if (stmt &&
//ScenSim-Port//			    (stmt -> op == send_option_statement ||
			    (stmt -> op == executable_statement::send_option_statement ||//ScenSim-Port//
//ScenSim-Port//			     (stmt -> op == on_statement &&
			     (stmt -> op == executable_statement::on_statement &&//ScenSim-Port//
			      (stmt -> data.on.evtypes & ON_TRANSMISSION)))) {
			    eptr = &config -> on_transmission -> statements;
//ScenSim-Port//			    if (stmt -> op == on_statement) {
			    if (stmt -> op == executable_statement::on_statement) {//ScenSim-Port//
...
parse_option_list(struct parse *cfile, struct option ***list)
{
	...
	*list = NULL;
	if (ix) {
//ScenSim-Port//		*list = dmalloc ((ix + 1) * sizeof(struct option *), MDL);
		*list = (struct option**)dmalloc ((ix + 1) * sizeof(struct option *), MDL);//ScenSim-Port//
		if (!*list)
...
//ScenSim-Port//void parse_interface_declaration (cfile, outer_config, name)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct client_config *outer_config;
//ScenSim-Port//	char *name;
void parse_interface_declaration (struct parse *cfile, struct client_config *outer_config, char *name)//ScenSim-Port//
{
	int token;
	const char *val;
...
//ScenSim-Port//				   name, isc_result_totext (status));
			log_fatal ("Can't record interface %s: %i", name, status);//ScenSim-Port//
...
//ScenSim-Port//void make_client_state (state)
//ScenSim-Port//	struct client_state **state;
void make_client_state (struct client_state **state)//ScenSim-Port//
{
...
//ScenSim-Port//void make_client_config (client, config)
//ScenSim-Port//	struct client_state *client;
//ScenSim-Port//	struct client_config *config;
void make_client_config (struct client_state *client, struct client_config *config)//ScenSim-Port//
{
...
//ScenSim-Port//void parse_client_lease_statement (cfile, is_static)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int is_static;
void parse_client_lease_statement (struct parse *cfile, int is_static)//ScenSim-Port//
{
...
//ScenSim-Port//void parse_client_lease_declaration (cfile, lease, ipp, clientp)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct client_lease *lease;
//ScenSim-Port//	struct interface_info **ipp;
//ScenSim-Port//	struct client_state **clientp;
void parse_client_lease_declaration (struct parse *cfile, struct client_lease *lease, struct interface_info **ipp, struct client_state **clientp)//ScenSim-Port//
{
	int token;
	const char *val;
	struct interface_info *ip;
...
//ScenSim-Port//		if (token != STRING && !is_identifier (token)) {
//ScenSim-Port//			parse_warn (cfile, "expecting key name.");
//ScenSim-Port//			skip_to_semi (cfile);
//ScenSim-Port//			break;
//ScenSim-Port//		}
//ScenSim-Port//		if (omapi_auth_key_lookup_name (&lease -> key, val) !=
//ScenSim-Port//		    ISC_R_SUCCESS)
//ScenSim-Port//			parse_warn (cfile, "unknown key %s", val);
//ScenSim-Port//		parse_semi (cfile);
		assert(false); abort();//ScenSim-Port//
		break;
	      case TOKEN_BOOTP:
		lease -> is_bootp = 1;
...
//ScenSim-Port//	pref = dmalloc(sizeof(*pref), MDL);
	pref = (dhc6_addr*)dmalloc(sizeof(*pref), MDL);//ScenSim-Port//
	if (pref == NULL) {
...
//ScenSim-Port//void parse_string_list (cfile, lp, multiple)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct string_list **lp;
//ScenSim-Port//	int multiple;
void parse_string_list (struct parse *cfile, struct string_list **lp, int multiple)//ScenSim-Port//
{
	int token;
	const char *val;
...
//ScenSim-Port//void parse_reject_statement (cfile, config)
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	struct client_config *config;
void parse_reject_statement (struct parse *cfile, struct client_config *config)//ScenSim-Port//
{
	int token;
	const char *val;
...
//ScenSim-Port//int parse_allow_deny (oc, cfile, flag)
//ScenSim-Port//	struct option_cache **oc;
//ScenSim-Port//	struct parse *cfile;
//ScenSim-Port//	int flag;
int client_parse_allow_deny (struct option_cache **oc, struct parse *cfile, int flag)//ScenSim-Port//
{
