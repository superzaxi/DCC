	...
	if (vfree == -1) {
		...
		if (vfree == g -> va_max) {
			if (g -> va_max)
				...
				vm_new = 10;
//ScenSim-Port//			va = dmalloc (vm_new * sizeof *va, MDL);
			va = (omapi_value_t**)dmalloc (vm_new * sizeof *va, MDL);//ScenSim-Port//
			if (!va)
				return ISC_R_NOMEMORY;
//ScenSim-Port//			ca = dmalloc (vm_new * sizeof *ca, MDL);
			ca = (u_int8_t*)dmalloc (vm_new * sizeof *ca, MDL);//ScenSim-Port//
			if (!ca) {
				dfree (va, MDL);
				return ISC_R_NOMEMORY;
			}
