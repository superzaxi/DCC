...
//ScenSim-Port//omapi_handle_table_t *omapi_handle_table;
//ScenSim-Port//omapi_handle_t omapi_next_handle = 1;	/* Next handle to be assigned. */
...
//ScenSim-Port//		omapi_handle_table = dmalloc (sizeof *omapi_handle_table, MDL);
		omapi_handle_table = (omapi_handle_table_t*)dmalloc (sizeof *omapi_handle_table, MDL);//ScenSim-Port//
		if (!omapi_handle_table)
...
//ScenSim-Port//		new = dmalloc (sizeof *new, MDL);
		new = (omapi_handle_table_t*)dmalloc (sizeof *new, MDL);//ScenSim-Port//
		if (!new)
			return ISC_R_NOMEMORY;
						  ...
						  omapi_object_t *o)
{
	...
	if (!inner) {
//ScenSim-Port//		inner = dmalloc (sizeof *inner, MDL);
		inner = (omapi_handle_table_t*)dmalloc (sizeof *inner, MDL);//ScenSim-Port//
		if (!inner)
			return ISC_R_NOMEMORY;
