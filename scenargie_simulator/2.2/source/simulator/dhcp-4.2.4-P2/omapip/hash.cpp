...
//ScenSim-Port//int new_hash_table (tp, count, file, line)
//ScenSim-Port//	struct hash_table **tp;
//ScenSim-Port//	unsigned count;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int new_hash_table (struct hash_table **tp, unsigned count, const char *file, int line)//ScenSim-Port//
{
...
//ScenSim-Port//	rval = dmalloc(sizeof(struct hash_table) +
	rval = (struct hash_table*)dmalloc(sizeof(struct hash_table) +//ScenSim-Port//
...
//ScenSim-Port//void free_hash_table (tp, file, line)
//ScenSim-Port//	struct hash_table **tp;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
void free_hash_table (struct hash_table **tp, const char *file, int line)//ScenSim-Port//
{
...
//ScenSim-Port//	int i;
//ScenSim-Port//	struct hash_bucket *hbc, *hbn = (struct hash_bucket *)0;
//ScenSim-Port//
//ScenSim-Port//	for (i = 0; i < ptr -> hash_count; i++) {
//ScenSim-Port//	    for (hbc = ptr -> buckets [i]; hbc; hbc = hbn) {
//ScenSim-Port//		hbn = hbc -> next;
//ScenSim-Port//		if (ptr -> dereferencer && hbc -> value)
//ScenSim-Port//		    (*ptr -> dereferencer) (&hbc -> value, MDL);
//ScenSim-Port//	    }
//ScenSim-Port//	    for (hbc = ptr -> buckets [i]; hbc; hbc = hbn) {
//ScenSim-Port//		hbn = hbc -> next;
//ScenSim-Port//		free_hash_bucket (hbc, MDL);
//ScenSim-Port//	    }
//ScenSim-Port//	    ptr -> buckets [i] = (struct hash_bucket *)0;
//ScenSim-Port//	}
//ScenSim-Port//#endif
...
//ScenSim-Port//struct hash_bucket *hash_bucket_hunks;
//ScenSim-Port//
//ScenSim-Port//void relinquish_hash_bucket_hunks ()
//ScenSim-Port//{
//ScenSim-Port//	struct hash_bucket *c, *n, **p;
//ScenSim-Port//
//ScenSim-Port//	/* Account for all the hash buckets on the free list. */
//ScenSim-Port//	p = &free_hash_buckets;
//ScenSim-Port//	for (c = free_hash_buckets; c; c = c -> next) {
//ScenSim-Port//		for (n = hash_bucket_hunks; n; n = n -> next) {
//ScenSim-Port//			if (c > n && c < n + 127) {
//ScenSim-Port//				*p = c -> next;
//ScenSim-Port//				n -> len++;
//ScenSim-Port//				break;
//ScenSim-Port//			}
//ScenSim-Port//		}
//ScenSim-Port//		/* If we didn't delete the hash bucket from the free list,
//ScenSim-Port//		   advance the pointer. */
//ScenSim-Port//		if (!n)
//ScenSim-Port//			p = &c -> next;
//ScenSim-Port//	}
//ScenSim-Port//		
//ScenSim-Port//	for (c = hash_bucket_hunks; c; c = n) {
//ScenSim-Port//		n = c -> next;
//ScenSim-Port//		if (c -> len != 126) {
//ScenSim-Port//			log_info ("hashbucket %lx hash_buckets %d free %u",
//ScenSim-Port//				  (unsigned long)c, 127, c -> len);
//ScenSim-Port//		}
//ScenSim-Port//		dfree (c, MDL);
//ScenSim-Port//	}
//ScenSim-Port//}
//ScenSim-Port//#endif

//ScenSim-Port//struct hash_bucket *new_hash_bucket (file, line)
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
struct hash_bucket *new_hash_bucket (const char *file, int line)//ScenSim-Port//
{
	struct hash_bucket *rval;
	int i = 0;
	if (!free_hash_buckets) {
//ScenSim-Port//		rval = dmalloc (127 * sizeof (struct hash_bucket),
		rval = (struct hash_bucket*)dmalloc (127 * sizeof (struct hash_bucket),//ScenSim-Port//
				file, line);
		if (!rval)
			return rval;
...
//ScenSim-Port//void free_hash_bucket (ptr, file, line)
//ScenSim-Port//	struct hash_bucket *ptr;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
void free_hash_bucket (struct hash_bucket *ptr, const char *file, int line)//ScenSim-Port//
{
//ScenSim-Port//#if defined (DEBUG_MALLOC_POOL)
//ScenSim-Port//	struct hash_bucket *hp;
//ScenSim-Port//
//ScenSim-Port//	for (hp = free_hash_buckets; hp; hp = hp -> next) {
//ScenSim-Port//		if (hp == ptr) {
//ScenSim-Port//			log_error ("hash bucket freed twice!");
//ScenSim-Port//			abort ();
//ScenSim-Port//		}
//ScenSim-Port//	}
//ScenSim-Port//#endif
...
//ScenSim-Port//	register const unsigned char *s = name;
	register const unsigned char *s = (const unsigned char*)name;//ScenSim-Port//
	int i = len;
	register unsigned c;
...
//ScenSim-Port//void add_hash (table, key, len, pointer, file, line)
//ScenSim-Port//	struct hash_table *table;
//ScenSim-Port//	unsigned len;
//ScenSim-Port//	const void *key;
//ScenSim-Port//	hashed_object_t *pointer;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
void add_hash (struct hash_table *table, const void *key, unsigned len, hashed_object_t *pointer, const char *file, int line)//ScenSim-Port//
{
	int hashno;
	struct hash_bucket *bp;
	void *foo;
...
//ScenSim-Port//	bp -> name = key;
	bp -> name = (const unsigned char*)key;//ScenSim-Port//
	if (table -> referencer) {
		foo = &bp -> value;
//ScenSim-Port//		(*(table -> referencer)) (foo, pointer, file, line);
		(*(table -> referencer)) ((hashed_object_t**)foo, pointer, file, line);//ScenSim-Port//
	} else
		bp -> value = pointer;
...
//ScenSim-Port//void delete_hash_entry (table, key, len, file, line)
//ScenSim-Port//	struct hash_table *table;
//ScenSim-Port//	unsigned len;
//ScenSim-Port//	const void *key;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
void delete_hash_entry (struct hash_table *table, const void *key, unsigned len, const char *file, int line)//ScenSim-Port//
{
	int hashno;
	...
	for (bp = table -> buckets [hashno]; bp; bp = bp -> next) {
		if ((!bp -> len &&
//ScenSim-Port//		     !strcmp ((const char *)bp->name, key)) ||
		     !strcmp ((const char *)bp->name, (const char*)key)) ||//ScenSim-Port//
		    (bp -> len == len &&
			...
			if (bp -> value && table -> dereferencer) {
				foo = &bp -> value;
//ScenSim-Port//				(*(table -> dereferencer)) (foo, file, line);
				(*(table -> dereferencer)) ((hashed_object_t**)foo, file, line);//ScenSim-Port//
			}
...
//ScenSim-Port//int hash_lookup (vp, table, key, len, file, line)
//ScenSim-Port//	hashed_object_t **vp;
//ScenSim-Port//	struct hash_table *table;
//ScenSim-Port//	const void *key;
//ScenSim-Port//	unsigned len;
//ScenSim-Port//	const char *file;
//ScenSim-Port//	int line;
int hash_lookup (hashed_object_t **vp, struct hash_table *table, const void *key, unsigned len, const char *file, int line)//ScenSim-Port//
{
	int hashno;
	struct hash_bucket *bp;
...
{
	size_t i;
//ScenSim-Port//	const unsigned char *s = v1;
	const unsigned char *s = (const unsigned char*)v1;//ScenSim-Port//
//ScenSim-Port//	const unsigned char *t = v2;
	const unsigned char *t = (const unsigned char*)v2;//ScenSim-Port//
