#include <assert.h>
#include <iostream>
//ScenSim-Port://#include <hash_set.h>  // Not Used!
#include <set>
#include <vector>
#include <fstream>
#include <algorithm>//ScenSim-Port://
//ScenSim-Port://#include <stdlib.h>
#include <cstdlib>//ScenSim-Port://
//ScenSim-Port://#include <math.h>
#include <cmath>//ScenSim-Port://
#include <climits>//ScenSim-Port://

#include "s_hull.h"

using std::abs;//ScenSim-Port://
using std::ios;//ScenSim-Port://
...
void write_Shx(std::vector<Shx> &pts, char * fname){
   ...
   int nr = static_cast<int>(pts.size());//ScenSim-Port://
   out << nr << " 2" << endl;
...
void write_Triads(std::vector<Triad> &ts, char * fname){
   ...
   int nr = static_cast<int>(ts.size());//ScenSim-Port://
   out << nr << " 7" << endl;
...
void T_flip2( std::vector<Shx> &pts, std::vector<Triad> &triads, int *slump, int numt, int start){
  ...
  //ScenSim-Port://double R,C,r0,c0, r1,c1, r2,c2, r3,c3, rot, rot2, dR, dC;
  //ScenSim-Port://int pa,pb,pc, pd, ab, bc, ac, D, L1, L2, L3, L4, T2;
  double R,C, r3,c3, rot, dR, dC;//ScenSim-Port://
  int pa,pb,pc, pd, D, L1, L2, L3, L4, T2;//ScenSim-Port://
...
void s_hull_del_ray2( std::vector<Shx> &pts, std::vector<Triad> &triads)
{
  ...
  int nump = static_cast<int>(pts.size());//ScenSim-Port://
  ...
  double c2 = pts[1].c;
  int mid = -1;
  double romin2 = 100000000.0, ro2;
  double R = 0.0;
  double C = 0.0;
  bool warningIsSetCheck = false;
  ...
  r = (pts[0].r + pts[1].r + pts[2].r )/double(3.0);//ScenSim-Port://
  c = (pts[0].c + pts[1].c + pts[2].c )/double(3.0);//ScenSim-Port://
  ...
  double dr, dc, rx,cx;
  Shx  ptx;
  int numt = INT_MAX;  // Bad value for "uninitialized variable warning"
  ...
  for( int k=3; k<nump; k++){
    ...
    int numh = static_cast<int>(hull.size()), numh_old = numh;//ScenSim-Port://
    ...
    for(int g=0; g<numh-1; g++){
      double gaf = -(hull[g+1].c-hull[g].c) * hull[g].tr + (hull[g+1].r-hull[g].r)*hull[g].tc;
      cerr << gaf << ' ' ;
    }
      ...
      if( df < 0 ){    // visible.
        ...
        for( int h=numh-2; h>0; h--){
          ...
          if( df < 0 ){  // h is visible
            ...
            h = static_cast<int>(hull.size()-1);//ScenSim-Port://
    ...
    int npx = static_cast<int>(pidx.size())-1;//ScenSim-Port://
    numt = static_cast<int>(triads.size());//ScenSim-Port://
    T0 = numt;
    ...
    if( npx == 1){
      ...
      if( hidx > 0 )
        ...
        numh = static_cast<int>(hull.size());//ScenSim-Port://
  ...
  //ScenSim-Port://cerr << "of triangles " << triads.size() << " to be flipped. "<< endl;
  ...
  int nits = static_cast<int>(ids.size()), nit=1;//ScenSim-Port://
  while(  nits > 0 && nit < 20){
    // char nam[128];
    ...
    nits = static_cast<int>(ids.size());//ScenSim-Port://
    nit ++;
...
void T_flip4( std::vector<Shx> &pts, std::vector<Triad> &triads, int *slump, std::set<int> &ids){
  ...
  while(x != ids.end() ){
    ...
    if( flipped == 0 && tri.ac >= 0 ){
      ...
      double scubit = dR*dR+dC*dC - tri.ro;
